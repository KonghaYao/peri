...
- Responses 请求诊断：`model_bridge.rs::generate_from_request` 消费 `PreparedModelRequest::responses_history_degraded`，每逻辑请求经 `StateEvent::ProtocolEvent → SystemNotification(level="warning")` 发一次安全通知；不写 transcript、system 或模型请求体，retry 不重复发射。`responses_lifecycle_test.rs` 覆盖换模型/endpoint/Chat/Anthropic 与同源静默，`responses_http_test.rs` 覆盖 incomplete→TurnError、默认 retry、多工具交错及真实 invoke 失败、取消和 usage 缺失/零值；`compact_v2/responses_native_test.rs` 覆盖 Full 后 SQLite 冷恢复再投影。
...
| 改 LLM 用量与失败统计 | `src/agent/model_bridge.rs` + `src/agent/stages/reason.rs` | bridge 累加 `ModelStreamEvent::AttemptUsage`（失败尝试）并把最终尝试用量取自 Completed（最后一个 Usage 快照兜底）；失败路径把总量写入 `ModelError::with_usage`；`reason.rs` 的 LlmCallEnd 使用 `Option<TokenUsage>`，失败路径读取 `failed_call_usage` | provider 未上报 = `None`（不得伪造成显式零）；重试调用 = 失败尝试 + 最终尝试各一次。回归：`responses_http_test.rs` 的 `responses_http_usage_preserves_missing_and_zero_on_event_boundary`、`responses_failed_with_usage_retries_and_keeps_each_attempt_usage`、`responses_incomplete_with_usage_never_retries_and_never_executes_tools` |
...
| 改 Responses 原生历史桥接与保留 | `src/agent/model_bridge.rs` + `src/agent/compact_v2/projection.rs` + `src/agent/stages/{reason,mod}.rs` | `decode_native_history` / `convert_model_block`；`ProviderCapabilities::openai_responses`；`project_message` 原生历史保护与 `validate_native_history_untouched`；`observed_llm_messages`；`AgentModelBridge::provider_capabilities` | 原生记录经 `ContentBlock::Unknown` 载体（tag `responses_native_history`）双向转换：只有确定 tag 才解码，损坏/未知版本 fail closed；含原生记录的消息不参与微压缩 block 投影，`native_history_must_be_whole` 时投影前后逐字复核；LlmCallStart 事件快照剥离 provider 私有载荷（请求体观测投影的密文/arguments 脱敏由 peri-model `PreparedModelRequest` 负责） |
