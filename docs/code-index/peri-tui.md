...
| 改 print stream-json 计量 | `src/cli_print.rs` + `src/cli_print_usage_test.rs` + `tests/print_exit.rs` | `PrintUsage::from_meta` / `PrintOutput::handle_session_update` / `stream_result` | `assistant.message.usage` 为每次非 replay 调用，`result.usage` 为累计，两者不得重复相加；内部完整 input 扣缓存后输出 Claude 四字段，无数据/成本保留 null；本地 provider 集成覆盖 Read 后完整历史、尾帧 usage 与进程退出 |
...
| Session 切换与 load reservation | src/acp_client/client/session.rs | `ensure_session` / `new_session` / `load_session` / `delete_session`；`SessionLoadReservation` / `reserve_session_load` / `open_prompt_after_session_loads` 保持同步 reservation 与 operation gate 的线性化；dirty 恢复在一次 load transition 内确认（`RecoveryRequired` data → `confirm_dirty_recovery` → `peri/session_reset_dirty` → 重新 load），source/target 与有效 cwd 在等待回答前固定 |
