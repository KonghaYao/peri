...
pub struct InstanceConfig {
    ...
    pub token: String,
    /// 数据目录（`~/.local/share/acp-hub/instance/`，0600）。
...
pub async fn run(config: InstanceConfig) -> anyhow::Result<()> {
        ...
        tokio::select! {
            evt = events_rx.recv() => {
                let Some(evt) = evt else {
                    tracing::warn!(target: "acp_hub::instance", "transport 任务退出，daemon 结束");
                    break;
                };
                match evt {
                    TransportEvent::Connected => {
                        tracing::info!(target: "acp_hub::instance", "ws 已建立，等待认证");
                    }
                    TransportEvent::Authenticated => {
                        authenticated = true;
                        tracing::info!(target: "acp_hub::instance", "认证通过，开始补推");
                    ...
                    TransportEvent::Disconnected => {
                        ...
                        state
                            ...
                            .expect("buffer mutex poisoned")
                            .rollback_all();
                        tracing::info!(target: "acp_hub::instance", "断线：进入缓冲模式");
                    }
                    TransportEvent::AuthTimeout => {
                        tracing::warn!(target: "acp_hub::instance", "握手超时（随后停止重连）");
                    }
                    TransportEvent::Stopped(reason) => {
                        ...
                        tracing::error!(target: "acp_hub::instance", reason = reason_str,
            ...
            _ = tokio::signal::ctrl_c() => {
                tracing::info!(target: "acp_hub::instance", "收到 SIGINT，优雅退出");
                shutdown_all(&state, &config).await;
                handle.shutdown();
                break;
            }
    ...
    tracing::info!(target: "acp_hub::instance", "daemon 退出");
    Ok(())
}
    ...
    match frame {
        Frame::InstanceSpawn(spawn) => {
            if !authenticated {
                ...
                tracing::warn!(target: "acp_hub::instance", chat_id = %spawn.chat_id,
                    "认证通过前收到 instance/spawn（丢弃，不执行）");
                return;
            }
        ...
        Frame::InstanceKill(kill) => {
            if !authenticated {
                state.pre_auth_dropped.fetch_add(1, Ordering::Relaxed);
                tracing::warn!(target: "acp_hub::instance", chat_id = %kill.chat_id,
                    "认证通过前收到 instance/kill（丢弃，不执行）");
                return;
            }
        ...
        Frame::InstanceForward(fwd) => {
            ...
            let ack = Frame::InstanceForwardAck(acp_hub_proto::instance::InstanceForwardAck {
        ...
        other => {
            tracing::warn!(target: "acp_hub::instance", tag = %other.tag(),
                "入站帧不在 instance 处理面（丢弃并计数）");
        }
    }
}
...
async fn handle_downlink(state: &HubState, chat_id: &str, frame: &serde_json::Value) -> bool {
    ...
    match acp {
        ...
        None => {
            tracing::warn!(target: "acp_hub::instance", chat_id,
...
async fn handle_spawn(state: &HubState, handle: &TransportHandle, spawn: InstanceSpawn) {
    ...
    if let Some(env) = &spawn.env {
        ...
        if let Err(cat) = validate_env(env, &extra) {
            tracing::warn!(target: "acp_hub::instance", chat_id = %sid, reason = cat,
                "spawn env 校验失败");
    ...
    if !Path::new(&spawn.cwd).is_dir() {
        tracing::warn!(target: "acp_hub::instance", chat_id = %sid, "spawn cwd 不存在");
    ...
    if idempotent_hit {
        tracing::info!(target: "acp_hub::instance", chat_id = %sid,
    ...
    match child::spawn(&spawn.cmd, &spawn.cwd, spawn.env.as_ref(), &sid, child_tx).await {
        Ok(acp) => {
            ...
            tracing::info!(target: "acp_hub::instance", chat_id = %sid, epoch, pgid,
                "ACP 进程启动");
            send_spawn_ack(handle, &command_id, &sid, true, None).await;
        }
        Err(e) => {
...
async fn handle_kill(
    ...
    match acp {
        Some(acp) => {
            ...
            tracing::info!(target: "acp_hub::instance", chat_id = %sid, grace_ms = grace.as_millis(),
                "kill 完成（进程组）");
        }
        None => {
            tracing::info!(target: "acp_hub::instance", chat_id = %sid,
...
async fn forward_child_output(
    ...
    match out {
        ChildOutput::Frame(evt) => {
            ...
            let (epoch, seq, buffered) = {
                ...
                let Some(entry) = chats.get_mut(&sid) else {
                    tracing::debug!(target: "acp_hub::instance", chat_id = %sid,
                        "帧属于未知 session（丢弃）");
                    return;
                };
            ...
            // 转发诊断（§9.3 脱敏：只记 method/type 与 seq，不记正文）。
            {
                let kind = evt
                    ...
                    .unwrap_or("?");
                tracing::debug!(target: "acp_hub::instance", chat_id = %sid, seq,
                    kind, buffered, "child frame forwarding");
            }
            // 单帧超限检查（在线/断线统一，§8.5：超限跳过 + gap，不做截断）。
            let payload = if online {
                serde_json::to_vec(&Frame::InstanceEvent(InstanceEvent {
                    chat_id: sid.clone(),
                    epoch,
                    seq,
                ...
                serde_json::to_vec(&acp_hub_proto::instance::BufferedFrame {
                    seq,
                    frame: evt.frame.clone(),
                })
            };
            ...
            if payload.len() > config.max_frame_bytes {
                ...
                tracing::warn!(target: "acp_hub::instance", chat_id = %sid, seq,
        ...
        ChildOutput::Exit { session_id, code } => {
            ...
            if authenticated {
                let frame = Frame::InstanceProcessExit(InstanceProcessExit {
                    chat_id: sid.clone(),
                    code,
                });
                ...
                tracing::debug!(target: "acp_hub::instance", chat_id = %sid, code,
                    "断线期间进程退出（重连后由对账呈现）");
            }
        }
        ChildOutput::DroppedNoSessionId => {
            let count = state.dropped_no_sid.fetch_add(1, Ordering::Relaxed) + 1;
            tracing::warn!(target: "acp_hub::instance", count, "帧无法提取 sessionId，丢弃（本地缺口计数）");
        }
    }
}
...
fn ring_push(state: &HubState, sid: &str, seq: u64, frame: serde_json::Value, cap: usize) {
    ...
    ring.push(acp_hub_proto::instance::BufferedFrame { seq, frame });
}
...
async fn resync_loop(state: &HubState, handle: &TransportHandle, _config: &InstanceConfig) {
    loop {
        ...
        let Some((sid, from_seq, frames, epoch)) = job else {
            ...
            if all_clear {
                tracing::info!(target: "acp_hub::instance", "补推完成，全部 session 转实时");
                return;
            }
        ...
        match handle.send_acked(frame).await {
            Ok(()) => {
                ...
                tracing::debug!(target: "acp_hub::instance", chat_id = %sid, from_seq,
...
#[allow(dead_code)]
fn ring_snapshot(state: &HubState, chat_id: &str) -> Vec<acp_hub_proto::instance::BufferedFrame> {
    state
        .rings
        .lock()
        .expect("rings mutex poisoned")
