...
use super::tool_projection::{project_tool_completion, project_tool_start};
// 保留既有 public 路径，投影规则由共享 module 持有。
pub use super::tool_projection::tool_result_content;
...
pub fn map_event(event: &ExecutorEvent, context_window: u32, caps: &PeriCaps) -> Vec<MappedEvent> {
    match event {
        ...
        ExecutorEvent::ToolStart {
            ...
            vec![MappedEvent::standard_with_src(
                vec![SessionUpdate::ToolCall(
                    project_tool_start(tool_call_id, name, input),
                )],
                source_agent_id.clone(),
            )]
        }
        ...
        ExecutorEvent::ToolEnd {
            ...
            update.fields.title = Some(name.clone());
            update.fields.raw_output = raw_output;
            vec![MappedEvent::standard_with_src(
