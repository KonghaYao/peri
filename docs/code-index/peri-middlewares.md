...
| 改条件注册 / 关闭面 | `src/assembly.rs`（条件分支集中在 :479-556） | `disabled.contains(名)` 关闭面（MetaHarness 连坐父工具集 :201-227）；条件注册：MCP 看 pool（:479-505）、Workflow 看 executor（:234-241）、LSP 看 servers（:521-548）、Goal 看 controller（:549-556） | 条件注册与 Hook 组展开按装配实现判断，蓝本只定槽位顺序；Hook 每非空 group 展开一个实例（:409-440）；workflow agent 链不装配 HITL（:855-865） |
...
| 改工具搜索核心白名单 / 元工具名 | `src/tool_search/core_tools.rs` | `CORE_TOOL_NAMES`（:64，14 项：Bash/Write/Edit/Read/Glob/Grep/folder_operations/Agent/WebFetch/WebSearch/AskUserQuestion/TodoWrite/SkillTool/DiscoverSkillsTool）；`SEARCH_EXTRA_TOOLS_NAME`/`EXECUTE_EXTRA_TOOL_NAME`（:6/:8）；`core_tools_sorted_csv`（:84） | 白名单驱动 ExecuteExtraTool description 动态生成（构造时一次性，P1-1）与声明覆盖完整性测试（declaration_test.rs:284）；改名须三处同步（core_tools 常量 + declaration 测试 + 提示词） |
| 改 deferred 搜索（索引 / 评分 / 声明） | `src/tool_search/tool_index.rs` + `keyword_search.rs` + `declaration.rs` | `ToolSearchIndex::build`（tool_index.rs:190）/`search`（:215）/`get_tool`（:292）/`format_deferred_list`（:306）；`keyword_score`（keyword_search.rs:81）；`collect_declarations`（declaration.rs:16） | 索引只收 `!is_direct()` 工具；查询按 camelCase / MCP 前缀切分（`parse_query` :57、`split_camel_case` :7、`split_mcp_prefix` :33）；声明段驱动提示词层（design v2 §2.5.3） |
| 改 deferred 执行（ExecuteExtraTool） | `src/tool_search/execute_tool.rs` | `ExecuteExtraTool`（:72，`new` :80）；`ExecuteExtraToolResolver::resolve`（:44，wrapper_name 透传）；`parse_extra_tool_call`（core_tools.rs:31）；`resolve_effective_tool_name`（core_tools.rs:51） | 从 `tool_name`/`params` 字段解包，目标必须在 shared_tools 表内（`DirectToolInvocationResolver` 查找 + `normalize_params`）；非 ExecuteExtraTool 调用直通不包装；Resolver 由装配 `build_tool_resolver`（assembly.rs:873）注入 |
| 改 ToolSearch 中间件注入 | `src/tool_search/middleware.rs` | `ToolSearchMiddleware`（:26）；`before_agent`（:64，索引构建 :100）；`prompt_contribution`（:60，读 cached_contribution） | 优先读 v2 每 turn 本地工具视图（`state.local_tools()`），无则回退 shared_tools（v1/测试路径）；deferred 非空且 content_version / count 变化才重建（P2-2 版本比对，:92-97）；数量变化 push_recall；collect_tools 提供 SearchExtraTools + ExecuteExtraTool + ArtifactTool（:52-58） |
| 改 artifacts 上传工具 | `src/tool_search/artifact_tool.rs` + `artifact_client.rs` | `ArtifactTool`（artifact_tool.rs:15，`new` :21）；`ArtifactClient`（artifact_client.rs:5，`from_env_or_default` :30、`upload_url` :37、`format_output` :86） | 工具名/namespace 见 :105/:114；上传地址来自 env 或默认值；结果格式化统一经 `format_output` |
| 改 MCP 连接 / 初始化 | `src/mcp/client.rs` + `src/mcp/initialize.rs` + `src/mcp/reconnect.rs` | `McpClientPool`（client.rs:166）；`run_initialize`（initialize.rs:17，合并配置 → 逐台 spawn）；`serve_client_auto`（client.rs:921）/`spawn_stdio_transport`（:1007）/`build_http_transport`（:1046）；`reconnect`（reconnect.rs:16） | 超时 STDIO 10s / HTTP 30s / shutdown 5s（client.rs:224-226）；初始化完成前状态写入不产生上下线通知（首 turn 概览已覆盖，避免重复事件）；`status_change_text`（:204）生成逐台通知文本 |
| 改 MCP 状态通知 / 缓冲 | `src/mcp/middleware.rs` + `src/mcp/client.rs`（pending_changes :182） | `McpMiddleware::first_turn_reminder`（middleware.rs:178，连接概览注入）；`before_model`（:272，drain pending_changes 成 Info 消息）；`push_status_changes`（:124） | 运行中变化进全局缓冲（任一会话消费一次即清空）；初始化中（initialized=false）的状态写入不通知，避免与首 turn 概览重复 |
...
| 改 MCP 工具 / 资源注册 | `src/mcp/tool_bridge.rs` + `resource_tool.rs` + `discover_tool.rs` | `build_tool_bridges`（tool_bridge.rs:207，pool → Vec<Box<dyn BaseTool>>）；`McpToolBridge`（:31，`new` :57）；`McpResourceTool`（resource_tool.rs:45）；`DiscoverMCPTool`（discover_tool.rs:32） | 工具/资源仅在 pool 可用时注册（装配 assembly.rs:479-505 条件分支）；`McpMiddleware::collect_tools`（middleware.rs:154）提供 Discover + Resource；注册变更必须同时检查 pool、资源与 bridge 路径 |
| 改 MCP skill 发现 | `src/mcp/skill_discovery.rs` | `run_discovery`（:74）；`mcp_route_entries`（:221）；`is_skill_scheme`（:821）；`refresh_entry_and_content`（:564）；`verify_and_build`（:736） | 经 `McpSkillRegistry`（peri-acp-types）注册；`skill://` scheme 资源拉取 + verify 后入注册表；带 CommandSource 路由（`finish_command_source` :130）；`McpSkillPlaceholder`（:195）占位命令 |
...
| 改 MCP OAuth / 凭证 | `src/mcp/oauth_flow.rs` + `auth_store.rs` + `callback_server.rs` + `client_oauth.rs` | `OAuthCallbackServer::bind`（callback_server.rs:31）/`wait_for_code`（:43）/`parse_code_from_url`（:144）；`FileCredentialStore`（auth_store.rs）；OAuth 流程 `spawn_oauth_flow` / `start_oauth_flow`（oauth_flow.rs） | 每台 server 最多一个活跃 flow（`active_oauth_flows`，client.rs:193）；授权码经 ACP `mcp/oauth_callback` RPC 回传 → host 装配面查 `pending_oauth_callbacks`（client.rs:191）投递；token 只落本机权限保护文件（ARC-SECRET-001） |
...
| 改 skills 注入 / 工具 | `src/skills/mod.rs` + `src/skills/tools.rs` + `src/subagent/skill_preload.rs` | `SkillsMiddleware`（mod.rs:98，`build_frozen_summary` :261、`format_discovery_protocol` :131、`resolve_roots_static` :277）；`SkillTool`（tools.rs:28）/`DiscoverSkillsTool`（:113）；`extract_skill_names_from_text`（skill_preload.rs:26） | 渐进式摘要注入：会话开始冻结（`with_frozen_summary`，ARC-FROZEN-001）；SkillTool 走 `find_skill_content` 统一查找入口；预加载以 fake `SkillTool(skill_name)` ToolUse→ToolResult 序列 `add_message` 注入（放用户消息后，不碰 prompt cache 前缀）；MCP skill 经 `with_mcp_registry` 接入 |
...
| 改 HITL 提问通道（AskUser） | `src/hitl/mod.rs` + `src/ask_user/mod.rs` | `HumanInTheLoopMiddleware`（hitl/mod.rs:39，`tool_names` :45、`new` :74、`collect_tools` :90）；`parse_ask_user`（ask_user/mod.rs:31）；`ask_user_tool_definition`（:65） | 2026-08-15 职责拆分：本中间件只做提问通道（AskUserQuestion 工具 + 12_ask_user 段落），审批归 PermissionMiddleware；broker 恒 None 时不装配（workflow agent 无提问，assembly.rs:855-865）；装配须用原始 broker（见多路复用行） |
| 改 permission 审批主链路 | `src/permission/mod.rs` + `shared_mode.rs` | `PermissionMiddleware`（mod.rs:178，`new` :221、`disabled` :235、`with_shared_mode` :246、`process_batch` :290 批量审批）；`with_broker_timeout`（:262） | 审批以解析后的 effective tool name 为准：`effective_tool_name` 是 `resolve_effective_tool_name` 的 re-export（mod.rs:168 ← tool_search/core_tools.rs:51，识别 ExecuteExtraTool 解包后的真实目标名）；包装/搜索/代理工具不得绕过审批；broker + permission_mode 均 Some 才启用，否则 Bypass（后台 agent 默认，assembly.rs:843-853） |
...
| 改 workflow 中间件 | `src/workflow/mod.rs` | `WorkflowMiddleware`（:36，`new` :55、`resume_workflow` :135、`create_tool` :92、`subscribe_notifications` :308、`progress_store` :108）；`WorkflowMiddlewareAdaptor`（:332，端口适配 + `collect_tools` :386） | executor 可用时才注册（装配 assembly.rs:234-241）；优先复用 session 级实例（progress_store/registry/runner 跨 turn 存活），无则临时实例（print 模式）；经 `WorkflowMiddlewarePort` 注入装配面；通知双路径见 docs/design/workflow-system.md §4（Path A bg-task-completed、Path B push_defer 唤醒） |
...
| 改 goal steering | `src/goal_middleware.rs` + `src/goal/tool.rs` | `GoalMiddleware`（goal_middleware.rs:24，`new` :33、`after_agent` :85、`render_steering` :45）；`GoalTool`（goal/tool.rs:17，deferred，is_direct 默认 false） | controller 可用才装配（链最后，assembly.rs:549-556）；goal active 且无既有 block_continue 时按 round 递增紧迫感模板注入（round 1/2/3+ 三档），必须以 `Human + <system-reminder>` 经 v2 MessageQueue `Defer` kind 注入（禁止 BaseMessage::system——会污染 frozen_system_prompt），并设 `block_continue = "goal_active"` 让 executor 自驱续跑 |
...
| 条件注册 / 禁用面 | `disabled.contains(名)`（:301-556）；MCP/Workflow/LSP/Goal 条件分支；Hook 组展开（:409-440） |
| broker / 端口还原 | effective_broker Multiplex 包装（:172-188）；downcast 还原端口实例（:160-166）；`build_tool_resolver`（:873，ExecuteExtraToolResolver 注入）；`build_workflow_middleware`（:895）；`build_error_suggest`（:877） |
...
| 搜索/声明/白名单 | keyword_search.rs（keyword_score :81）；declaration.rs（collect_declarations :16）；core_tools.rs（CORE_TOOL_NAMES :64 / resolve_effective_tool_name :51）；artifact_tool.rs / artifact_client.rs |
...
| 功能 | 入口/关键点 |
| --- | --- |
| 连接 / pool | client.rs（McpClientPool :166、serve_client_auto :921、spawn_stdio_transport :1007、build_http_transport :1046）；initialize.rs（run_initialize :17）；reconnect.rs（reconnect :16）；transport.rs（TransportConfig :9） |
| 配置合并 | config.rs（load_merged_config_full :223 / load_merged_config :349 / remove_server_from_config :385 / set_server_disabled :473） |
| 工具 / 资源 / skill | tool_bridge.rs（build_tool_bridges :207）；resource_tool.rs（McpResourceTool :45）；discover_tool.rs（DiscoverMCPTool :32）；skill_discovery.rs（run_discovery :74） |
| 中间件 | middleware.rs（McpMiddleware :24，collect_tools :154 / before_agent :196 / before_model :272 / first_turn_reminder :178） |
...
| 中间件 / 摘要 | mod.rs（SkillsMiddleware :98 / build_frozen_summary :261 / format_discovery_protocol :131 / global_config_path :28） |
| 工具 | tools.rs（SkillTool :28 / DiscoverSkillsTool :113）；builtin/（BuiltinSkill :14 / parse_builtin_frontmatter :50） |
...
| 功能 | 入口/关键点 |
| --- | --- |
| 工具 trait | `peri-acp-types/src/tools.rs`（BaseTool，is_direct 默认 false）；注册面 = 各 middleware `collect_tools()`：filesystem.rs:39、mcp/middleware.rs:154、lsp/middleware.rs:54、subagent/mod.rs:546、hitl/mod.rs:90、cron/middleware.rs:29、tool_search/middleware.rs:52、goal_middleware.rs:76 等 |
