...
| **R38** | **第二轮收口的交付期登记（R36 的延后项闭合 + 矩阵外测试文件 owner 补充）**：① R36 记录的卫生项**已执行** —— `middleware/web_search.rs` / `middleware/web_fetch.rs` / `artifact/tool.rs` 三处 `is_direct() -> true` 覆盖删除（三者的活构造点只有 builtin server 内部与测试；「提供面已消失」由 acceptance §12 命令 46 的首个请求名单作证：effective name 在、裸名不在 ⇒ direct 身份由声明表的逐工具 `direct` 决定）② 同两文件为让「真实 HTTP 形态」在无外网条件下可覆盖而做的**测试可见**改造：`WebSearchTool` / `WebFetchTool` 字段化（`base_url` / `timeout`）+ 三个 `#[cfg(test)] pub(crate)` 构造器。`new()` 仍用原常量、URL 拼接 `trim_end_matches('/')` 对无尾斜杠常量恒等 ⇒ **生产行为逐位不变，不新增公开 API、不新增配置项**；③ §4 矩阵外**测试文件**的 owner 补充（登记后视为矩阵内，owner = 收口编排者）：`peri-middlewares/src/mcp/builtin/web_test.rs`、`middleware/web_test.rs`、`middleware/web_fetch_test.rs`（三者承担 acceptance §12.3 第 6 行的本地真 HTTP 三形态）、`goal_middleware_test.rs`（承担第 7 行的 Goal 反证）。四者均为测试模块/测试文件形态，不动生产契约 | 本文件 R36 + §4 + acceptance §12（命令 41–58） |
| **R39** | **第三轮（独立复核后）的交付期登记**：① **提供面矛盾消除** —— `peri-middlewares/src/artifact/mod.rs` 里的活体 `pub struct ArtifactMiddleware`（`Middleware` impl + `Default` + `collect_tools`）删除：`docs/code-index/peri-middlewares.md` 在 wave 1 已记「`ArtifactMiddleware` 已删除」，代码未删 ⇒ 属 doc/代码矛盾而非风格项；删除安全（`ChainSlot::Artifact` 已删、无活引用，`cargo check --workspace --all-targets` 0 error）② **断言强度补齐** —— `mcp::builtin_runtime::same_pool_instances_never_cross_wires_and_reconnect_touches_one_link` 的两条夹具 client 半边此前不进 `pool.services` ⇒ 「重连只动被点名链路」在该用例内无失败模式；现经 `TappedLink::hand_service_to_pool()` 真的登记进表，并新增 `converge_task → Quit` 的因果断言（重连确实关掉了被点名实例的旧链路）③ **文档同步（DOC-UPDATE-001）** —— `docs/design/meta-harness.md`、`docs/meta-harness.md`（两份拆两表：`MIDDLEWARE_NAMES` ∪ `BUILTIN_INSTANCE_POLICY_KEYS` = 已知键）、`docs/design/tool-system.md`、`docs/code-index/peri-middlewares.md` 四处把「Web / Artifact 由 middleware 提供 / `is_direct() = true` / `ArtifactMiddleware` 注册 ArtifactTool」改为现行事实；`middleware/web.rs` 的「若文件仍在磁盘上即死文件」改为「已删除」④ **pre-existing 发现（不在本波次修复）** —— `peri-tui` 的全局 atom 用例在并行全量里偶发失败（本轮 3 次并行全量命中 1 次；隔离复跑与单 crate 复跑为绿），机理与判定见 acceptance §12.7.3；属 `peri-tui` owner 面，按 §9 规则 1 不跨界修改，仅登记 | 本文件 R36/R38 + §4 + acceptance §12.7（命令 35、41–58 复跑） |
| **R40** | **第四轮（第三次独立复核）的交付期登记**：独立只读复核（`code-reviewer`，快照 2026-09-26 17:02）报 **12 项（0 blocker / 2 major / 10 minor）**，全部在 acceptance §12.8 处置完毕：① **记录更正** — §12.1「9 个文件：生产 3」漏计 `artifact/mod.rs`（§12.8.1 按轮次给出：§12.1–§12.7 = 17 个文件 = 生产 4 / 测试 6 / 文档 5 / 记录 2；§12.8 追加 3 个 = 文档 1 + 注释 2；合计 **20** 个 = 代码 12（含注释-only 2）/ 文档 6 / 记录 2）；§12.2 末句的 `passed ≥ 3` 只对 15 条测试命令成立；§12.4「10 项」→**9 项**＋末句 caveat；§12.5 标题「一一对应」→「逐条对应（末尾两项属 §8.1 非目标）」；§12.7.1 F2 证据列的「只剩两处」穷尽性声称撤销；§12.3 第 4 行 `mod_test.rs:507`→`:509`（→ **该行号变更本身方向错误**：实测 505 为文档注释、506 为 `#[test]`、507 为 `fn`，已在 R42 ②／acceptance §12.9.1 回到 `:507`） ② **悬空指路更正** — §12.3 第 3 行括注称「生产 handler 同 pool 不串」由 `StartupFixture` 承担，而代码自述该命题**无证据面**；现改为「不宣称」并在 §12.5 补该条，与用例自述三处同口径（**不改用例文本**）③ **同义反复撤销** — Goal 面追加断言被既有 `len == 1` / `name() == "goal"` 蕴含 ⇒ 撤销「可证伪替代」定性，§12.3 第 7 行改为按「无独立可证伪面」的**设计事实**登记，§12.4 缺口计数改 3 项 ④ **现行文档同步（DOC-UPDATE-001 续）** — `docs/design/meta-harness.md`（Artifact 承载句 + 示例 `HashSet<String>`）、`docs/design/middleware-system.md`（槽位 26→**25**、删 `#13 web` / `#24 artifact` 两行并按链蓝本重编号 1–24、同步全部编号引用、新增 Web/Artifact 由 `mcp` 槽位客户端侧 builtin 实例提供的脚注）、`docs/code-index/peri-middlewares.md`（`web.rs` 删除归属 wave 1 提交 `354705e5`）⑤ **失效注释清理** — `mcp/builtin/web.rs` 与 `mcp/builtin_runtime_test.rs` 的「无法在单测内指向本地桩」（本批已加 `with_endpoint_for_test` 与真 HTTP stub 用例）、`middleware/mod.rs` 的「S-02 未落地前 lib 测试报 E0432」（`declaration_test.rs` 已改走 `BUILTIN_MCP_INSTANCES`）⇒ 三处删除或改写 ⑥ **接受现状 1 项** — `example/minimal/README.md` 表头分组名义问题（17:01 的 Note 已声明两键为 builtin 实例关闭键，示例行为不变）⑦ **pre-existing 缺口复现（非本轮发现，不修复）** — 命令 62 首跑再次命中 `peri-tui` lib 偶发失败（exit 101；失败用例名因脚本 `tail -60` 截断**不可回读**），隔离连跑 3 次均 `1677 passed` 全绿；现象只在 workspace 级全量出现（范围收窄，仍不作因果断言）。判定口径不变（R39 ④），新现场数据与统计见 acceptance §12.8.6。**注**：本轮四处 `.rs` 改动**全部是注释 / 文档注释**，无可执行语句、断言、类型或签名变化 | 本文件 R38/R39 + §4 + acceptance §12.8（命令 35、59–69 复跑（区间内 **64 未使用**，见 acceptance §12.8.5 编号说明 / §12.9.2 L3）；69 = 记录末次落盘后的 `git diff --check` 复检） |
| **R41** | **第五轮（文档面追加更新）的交付期登记**：收口提交 `6eb0ca45` 之后按「更新文档」口径做两处**纯文档**修正（不改代码 / 测试 / 分级）① `example/minimal/README.md:37` 表头 `### Middleware controls` → `### Middleware and builtin instance controls`：该表内两行现属 builtin 实例关闭键（`BUILTIN_INSTANCE_POLICY_KEYS`），m12 由「接受现状」改为已更新；Note、两行含义与示例配置行为（`example/minimal/.peri/settings.json:26` / `:37`）不变 ② `docs/code-index/peri-middlewares.md:3` 更新注记的**轮次归属**更正：「`artifact/mod.rs` 的 `ArtifactMiddleware` 提供面 + 三处 `is_direct()` 覆写删除」由「收口第二轮」改为「收口第二～四轮（提交 `6eb0ca45`）」（与 m10 同型；证据：`git show 354705e5:peri-middlewares/src/artifact/mod.rs` 仍含 5 处 `ArtifactMiddleware`、`354705e5` 的 `middleware/web_search.rs` 仍含 `fn is_direct`）。**明确排除**：`docs/design/mcp-adaptation-v4-part-1.md` 不动 —— 该文件自述「已批准目标设计」，第 234 行明文不保存执行状态 / 提交号；其迁移清单的「迁移状态」列是**目标归属** | 本文件 R40 + acceptance §12.8.7（命令 70 复检） |
| **R42** | **第六轮（第四次独立复核：对第四轮 12 项处置的验证）的交付期登记**：独立只读复核（`code-reviewer`；只读、不跑 cargo）对 §12.8 的 12 项**处置**逐项验证 ⇒ **11 项 PASS / 1 项 FAIL**，另报 3 项一致性遗留；连同本节自查追加 1 项，**共 5 项全部在 acceptance §12.9 处置完毕**（无一项挂账）① **FAIL 回退（L1）** — m11 的「修复」采信了上一轮复核报告的行号锚点而**未回到代码文件**：报告称 507 是文档注释行、`#[test]` 在 508、`fn` 在 509，实测 `peri-middlewares/src/subagent/mod_test.rs` 为 505 = 文档注释、506 = `#[test]`、507 = `fn mutation_tool_matches_original_name_policy_for_builtin_names()`、509 = `for instance in BUILTIN_MCP_INSTANCES {`（该文件不在本批 20 文件内、行号自始未变）⇒ §12.3 第 4 行**回到 `:507`** 并更正括注；**不改用例文本**；§12.8.3 m11 与本表 R40 ① 保留原文并加前向指针 ② **三项一致性遗留（L2 / L3 / L4）** — 命令编号在 63 与 65 之间跳空：acceptance §12.8.5 表尾加**编号说明**（本表无命令 64、**不补造命令**、**不重编号**，区间写法按「64 未使用」读；本表验证列同步加注）；§12.8.6 第 3 条「`--workspace` 级全量 lib 共 4 次」→「共 5 次」（枚举项与括注均不变，仅纠笔误，**不**新增观察数据）；§12.8.1 第四轮注释面计数口径（「注释 2」只列新增文件 ⇒ 补注另 2 处落在已计入 17 的既有文件内（`goal_middleware_test.rs` = m3、`builtin_runtime_test.rs` = m7），第四轮注释面共 **4** 个 `.rs` 与 §12.8.5 末段一致；**合计算式 17 + 3 = 20 不变**）③ **自查追加（L5）** — acceptance 头部「证据时点」字段补「截至该时点未提交 + 其后已提交的提交号（`354705e5` / `6eb0ca45` / `9919096f`）」：该字段只描述 §1–§11 的采集时点，**不**重写任何小节 ④ **使用者文档（独立于本波验收分级）** — 新增 `docs/reference/artifact-remote-storage.md`（`artifact` 上传目标换到自建远程存储：两个环境变量、服务端 `POST /upload` 对接契约、上传限制与失败前缀、三条关闭路径的**真实文件形状**、边界；按 DOC-REFERENCE-001 声明不具权威性并路由 `docs/reference/mcp-ecosystem.md` §9.8；不含真实 token / 认证 URL / OAuth 值）+ `docs/reference/README.md` 索引一行 ⑤ **不触碰任何 `.rs`** ⇒ 按 R39 ④ / R40 口径不触发 `fmt` / `clippy` / 测试的复跑义务；复检见 acceptance §12.9.4 的命令 71（`git diff --check` 无输出）/ 72（文档本地链接检查无失效） | 本文件 R40/R41 + acceptance §12.9（命令 71/72 复检） |
