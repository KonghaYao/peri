...
| 改事件消费（新增/变更事件） | `src/kit/acp_events/` + `acp_notifier.rs` + `acp_bridge.rs` | `dispatch_and_notify`（acp_events/mod.rs:301，穷尽 match 分派到 streaming/tool/turn/system/subagent/agent/compact handler）；`AcpEventData`（acp_types.rs:1170，decode 映射 `{event,data}`）；渲染产出 `push_view_models`（render.rs:25）/`push_acp_state`（render.rs:615） | 终止事件（TurnDone/TurnInterrupted）必须离开 loading（turn.rs:12/:60，stale 用 request_id 配对）；transport 死亡兜底复位 loading（acp_notifier.rs:71）；`compact_just_completed` 由任何流事件清除（mod.rs:319）；契约 ARC-EVENT-001 |
