...
//!   分类规则为**信封结构性分类**【决策】（machine 是 dumb pipe，§3.3 禁止
...
use acp_hub_proto::machine::BufferedFrame;
...
impl DiskSegment {
    fn open(dir: &Path, session_id: &str) -> std::io::Result<Self> {
        fs::create_dir_all(dir)?;
        let path = dir.join(format!("{session_id}.buf"));
...
/// 全局缓冲：`session_id → SessionBuffer` 分桶 + 预算核算（§8.5 合计口径）。
pub struct Buffer {
...
impl Buffer {
    ...
    pub fn push(
        ...
        let entry = self
            ...
            .or_insert_with(|| SessionBuffer::new(session_id));
        ...
        if bytes.len() > self.max_frame_bytes {
            ...
            tracing::warn!(target: "acp_hub::machine", session_id, seq, bytes = bytes.len(),
            ...
            && entry.mem.len() < self.mem_frames_limit
        {
            ...
            if entry.disk.is_none() {
                match DiskSegment::open(&self.dir, session_id) {
                    Ok(d) => entry.disk = Some(d),
                    Err(e) => {
                        tracing::error!(target: "acp_hub::machine", session_id,
                            "缓冲磁盘段打开失败: {e}");
                        return PushOutcome::Oversize;
                    }
                }
    ...
    /// 一次 commit 清空整批）。
    pub fn commit(&mut self, session_id: &str) {
    ...
    pub fn rollback(&mut self, session_id: &str) {
        if let Some(entry) = self.sessions.get_mut(session_id) {
...
impl Watermark {
    ...
    pub fn load(data_dir: &Path) -> Result<Self, WatermarkError> {
        ...
        let state = match fs::read_to_string(&path) {
            Ok(content) => serde_json::from_str(&content).unwrap_or_else(|e| {
                tracing::warn!(target: "acp_hub::machine", path = %path.display(),
                    "水位文件损坏，按空水位处理: {e}");
    ...
    pub fn epoch_of(&self, session_id: &str) -> Option<u64> {
        self.state.sessions.get(session_id).map(|s| s.epoch)
    }
