...
export function registerGetmanRoutes(app: Hono) {
  ...
  app.post("/api/getman/parse-curl", async (c) => {
    try {
      ...
      try {
        ...
        return c.json({ error: "解析失败: " + (err.message || String(err)) }, 400);
      }

      if (!parsed.url) {
        return c.json({ error: "未找到 URL" }, 400);
      }
  ...
  app.post("/api/getman/proxy", async (c) => {
    try {
      const body = await c.req.json() as GetmanProxyBody;
      ...
      if (!allowedMethods.includes(upperMethod)) {
        return c.json({ error: "不支持的 HTTP 方法: " + method }, 400);
      }
      ...
      try {
        ...
        return c.json({ error: "请求失败", detail: err.message || String(err), time: Math.round(performance.now() - start) });
      }
      ...
      return c.json({ status: resp.status, statusText: resp.statusText, headers: responseHeaders, body: responseBody, contentType, size, time: elapsed });
    } catch (err: any) {
      return c.json({ error: err.message }, 500);
    }
  });
