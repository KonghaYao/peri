...
use super::{InteractionPrompt, events::AgentEvent};
...
pub struct BgTaskState {
    ...
    /// Agent 尚未 Done 但后台任务已完成的结构化结果缓存。
    /// 跨轮次保留——`submit_message` 开头 drain 合并到本轮 bgResults，
    /// `reset_for_new_round` 不清空此字段。
    pub pre_done_results: Vec<BgTaskResult>,
}

impl BgTaskState {
    /// 用户主动发起新一轮提交时清理 transient continuation 状态。
