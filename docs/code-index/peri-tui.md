...
- 数据流：`ACP transport → acp_client pump（普通 notification 保留 wildcard；已知 reverse interaction 先 strict exact-active gate，拒绝/投递失败用 typed cancel 结算）→ acp_notifier（interaction 原子封装 RequestId + payload，不写 UI state）→ acp_bridge（HITL/AskUser 再做 non-empty exact active session gate）→ dispatch_and_notify（发布 composite atom + durable block）→ VIEW_MODELS/ACP_STATE → components；response action 使用 block/composite 冻结 ID，并 compare-and-clear active surface`
...
| 改 ACP 请求发送 / reverse interaction 早期路由 | `src/acp_client/client.rs` + `interaction_response.rs` | `AcpTuiClient`；`spawn_pump` / `plan_reverse_request`；`permission_*_response` / `elicitation_cancel_response`；`prompt`、`cancel`、`new_session`、`load_session`、`send_raw_request` | 全部交互经 MpscTransport；普通 notification 使用 current=None/empty host wildcard，Permission/Elicitation 使用 method-specific session 提取与 strict exact-active gate，拒绝或 notifier send failure 以 typed cancel 结算；完整 pending/session transition ownership 不在 pump early gate 内 |
