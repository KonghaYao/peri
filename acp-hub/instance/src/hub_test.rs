...
fn valid_auth_response(hello: &InstanceHello) -> acp_hub_proto::conn::AuthResponse {
    let nonce: [u8; CHALLENGE_NONCE_LEN] = base64::engine::general_purpose::STANDARD
        .decode(&hello.nonce)
        .unwrap()
        .try_into()
        .unwrap();
    let context = acp_hub_proto::hmac::generate_connection_context();
...
fn spawn_frame(command_id: &str, chat_id: &str, script: &str) -> Frame {
...
fn kill_frame(command_id: &str, chat_id: &str, grace_ms: Option<u64>) -> Frame {
...
async fn handshake_server(ws: Ws) -> (SplitSink, SplitStream, InstanceHello) {
    let mut ws = ws;
    let hello = loop {
        match ws.next().await {
