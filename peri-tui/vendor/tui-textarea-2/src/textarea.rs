...
impl<'a> TextArea<'a> {
    ...
    /// Returns the current scroll position of the textarea, as `(top_row, top_col)`.
    ///
    /// `top_row` is the number of screen lines scrolled from the top (vertical scroll).
    /// `top_col` is the number of display columns scrolled from the left (horizontal scroll).
    ///
    /// This is useful for positioning a terminal cursor (e.g. for IME composition window)
    /// to match the visual cursor position after accounting for scroll.
    ///
    /// # Examples
    ///
    /// ```
    ...
    ///
    /// let mut textarea = TextArea::default();
    /// textarea.insert_str("a".repeat(100).as_str());
    /// // After rendering in a narrow area, the textarea would have scrolled horizontally
    /// let (top_row, top_col) = textarea.scroll_top();
    /// assert_eq!(top_row, 0);
    /// # // top_col depends on render area width, can't assert exact value here
    /// ```
