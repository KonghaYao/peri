...
Bash 的 `timeout` 单位为毫秒，前台（同步）执行恒有界：默认 15 秒、硬上限 120 秒（`0` 与超过
上限的请求都界到上限，未传或改写时在超时回执里说明）；到达上限不杀进程，而是提升为后台任务
并回传 `task_id`、`pid` 与实时日志路径。后台（`run_in_background: true`）未传或 `0` 表示不超时，
显式正超时按 1 毫秒至 600 秒 clamp。
