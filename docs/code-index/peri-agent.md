...
| 改 PTC 内部 effective-tool dispatch | `src/agent/stages/tool_dispatch.rs` + `peri-acp-types/src/tools.rs` | `StageEffectiveToolDispatcher::dispatch`；`collect_tool_results` | 从当前 runtime tool snapshot canonical resolve；内部 `tools.*` 复用 effective-name permission、事件、timeout、cancel；外层 `run_code` 独立按 Bash 同级审批，Node 原生 API 不经过该 seam |
