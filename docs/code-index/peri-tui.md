...
| subagent 详情面板内容源 | kit/panels/subagent_detail.rs + kit/bg_task_live.rs | `resolve_selected_subagent`：bg 任务以 `BG_LIVE_DETAIL` 为准，VIEW_MODELS 扫描只作回退（`find_selected_subagent`）；选中 id 为组 instance_id（消息区 Enter）或 agent_id（底栏行点击），经 `BG_DISPLAY.linked_agent_id` 解析 task_id | bg 的工具事件不进组、组在 turn 边界（`TurnSuspended`/`TurnInterrupted`）被归档冻结，内容只累积在 `BG_LIVE_DETAIL`——以 VIEW_MODELS 优先会让面板停在下线那一刻；回归见 `bg_task_live_test.rs::test_bg_group_frozen_in_view_models_after_turn_suspended` |
