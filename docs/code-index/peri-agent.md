...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-09-10（compact 压力样本与 Full re-inject 语义校准）
...
| 改 Micro/Full 执行细节 | `src/agent/compact_v2/{micro,projection,full}.rs` | `micro_compact`；`render_llm_view`；`full_compact_inner`；`re_inject_v2`、`extract_file_info`、`extract_skill_names` | Micro 不原地截断 transcript，而是持久化 message-level projection directive，后续模型视图由 `render_llm_view` 投影工具结果；Full 仅按本轮新 `excluded` transition 统计 affected messages，文件 re-inject 只从 Full 前可见 `Read` 来源收集，新 `Read` 同路径仍可注入更新内容；精确载荷污染风险见 `spec/issues/2026-09-09-p0-micro-compact-edit-write-context-corruption.md`，compact churn 调查见 `spec/issues/2026-09-10-p0-full-micro-compact-churn.md` |
