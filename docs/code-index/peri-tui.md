...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-09-08（System Reminder canonical view model）
...
| --- | --- | --- | --- |
| 改 System Reminder 展示 | `src/kit/acp_types/event_data.rs` + `src/kit/acp_events/system.rs` + `src/kit/tui_render_unit/reminder.rs` + `src/kit/message_area/render/user.rs` | `AcpEventData::SystemReminder`；canonical event handler；`TuiSystemReminder::from_wire` / `legacy`；reminder render | canonical 路径只按 DTO 的 category/source/kind/severity/delivery/audiences 建模，不扫描正文；`detect_reminder` 仅处理无结构化事件的 legacy 文本 fallback；reminder 独立于用户气泡 |
