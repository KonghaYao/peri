...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-09-11（只读打开错误分类、`close` 收尾与子会话冻结继承上下文）
...
| SQLite 会话存储 | src/sessions/sqlite_store.rs | `SqliteThreadStore`（:117）；角色映射 `role_of`（:395）；标题提取 `extract_title`（:448） |
