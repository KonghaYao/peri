//! Compact Pipeline — `/compact` 命令路径的 v2 实现（3.0 批 2 归位）。
//!
//! 归位说明：本模块承载深绑 Agent 层执行类型（`MessageTranscript` /
//! `compact_v2::run_compact`）的 `/compact` 命令执行体；协议面
//! `crate::session::command::compact::pipeline` 保留 re-export 薄壳。
//! 本模块对 Agent 层类型保留全路径引用（过渡宿主豁免至 L5——见
//! `spec/issues/2026-08-05-3.0-acp-events-session-batch2.md`），
//! 执行本体随 L5 executor 拆分物理迁入 peri-agent（届时协议面经
//! Controller 发起）。
//!
...
use crate::session::command::compact::events::{
...
pub async fn run_pipeline(ctx: CommandContext) -> PipelineOutcome {
    ...
    let mut transcript = peri_agent::session::transcript::MessageTranscript::new();
...
async fn run_v2_compact_with_cancel(
    transcript: &mut peri_agent::session::transcript::MessageTranscript,
...
) -> Result<peri_agent::agent::compact_v2::CompactResult, CancelOrError> {
    let result = tokio::select! {
        r = peri_agent::agent::compact_v2::run_compact(
            transcript,
            Some(model),
            config,
...
pub fn assemble_compact_messages(
    transcript: &peri_agent::session::transcript::MessageTranscript,
    ...
    if let Some(summary_index) = messages.iter().position(|message| {
        matches!(message, BaseMessage::Human { .. })
            && message
                .content()
                .contains(peri_agent::agent::compact_v2::CONTINUATION_HINT)
    }) {
        let summary = messages.remove(summary_index);
        messages.insert(0, summary);
    }

    let files = peri_agent::agent::compact_v2::extract_file_info(&messages);
    let skills = peri_agent::agent::compact_v2::extract_skill_names(&messages);
...
pub async fn execute_compact(
    ctx: crate::session::command::CommandContext,
) -> crate::session::command::CommandResult {
    match run_pipeline(ctx).await {
        PipelineOutcome::Completed { messages } => crate::session::command::CommandResult {
            messages,
            stop_reason: PromptStopReason::EndTurn,
        },
        PipelineOutcome::Cancelled { history } => crate::session::command::CommandResult {
