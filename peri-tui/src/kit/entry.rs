...
/// spawn kit 三链路（notifier / bridge / submit_consumer），进入 ratatui-kit 全屏。
///
...
pub async fn run_kit_fullscreen(
    ...
    // 4. 进入 ratatui-kit 全屏 event loop（fullscreen 自管 raw mode + alt screen）
    let result = element!(AppShell).fullscreen().await;

    // 5. 退出前触发 shutdown，让后台任务干净退出
    shutdown.cancel();

    // 6. teardown：hooks/MCP/Langfuse（与 legacy run_app 对称）
