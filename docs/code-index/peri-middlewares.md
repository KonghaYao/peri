...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-08-23（PTC artifact 0.2.3、deferred canonical target、skill aliases 与 ESM-only 契约同步）
...
| 改 PTC / `RunPtcCode` | `src/ptc/mod.rs` + `peri-js-runtime/src/executor.rs` + `npm-packages/@peri-ptc/`；canonical seam 在 `peri-agent/src/agent/stages/tool_dispatch.rs` | `RunPtcCode` invoke；`PtcRouter::route`；`EffectiveToolDispatcher::dispatch`；`ptc/start` | canonical `RunPtcCode` 为 deferred-only，经 `SearchExtraTools → ExecuteExtraTool` 执行；direct tools 不受影响。旧 `run_code` 不可执行，仅作为搜索迁移关键词。执行环境 ESM-only，Node module 只能使用动态 `await import('node:...')`，static `import` / `require` 不可用；artifact 固定为 `@peri-code/ptc@0.2.3`，Cargo `build.rs` 从 TypeScript 源码生成 `OUT_DIR` artifact 并由 Rust 内嵌，npm 发布独立生成 gitignored `dist`；缓存 identity+hash/private temp/`node <entry>`/source 前 protocol+build handshake 必须同步；默认 fail closed，仅显式环境变量允许精确版本 `npx` fallback；工具错误仅投影 stable code + fixed safe message；policy/HITL/event/tool card 投影 effective target并复用 timeout/cancel；assistant raw wrapper call 仅保留协议配对 |
