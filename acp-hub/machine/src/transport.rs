...
//!
//! - **连接**：`tokio_tungstenite::connect_async`（URL 路径 `/machine`）；发送/
...
//!   `whitelist::m1_check(tag, Role::Machine, Direction::Outbound)`（防异常帧）；
...
/// transport 配置（hub 从 `MachineConfig` 派生，§10）。
#[derive(Debug, Clone)]
...
impl TransportHandle {
    ...
    /// 发送帧并等待 writer 实际写入成功（machine/event 与 buffer_sync 批次用；
    ...
    async fn dispatch(&self, out: Outbound) -> Result<(), SendError> {
        ...
        match try_dispatch(tx, out) {
            ...
            Err(SendError::QueueFull) => {
                tracing::warn!(target: "acp_hub::machine", "发送队列已满（§8.6 硬阈值），关闭连接走重连补推");
                *guard = None;
...
where
    F: Fn() -> (AuthSession, MachineHello) + Send + Sync + 'static,
{
    ...
    loop {
        ...
        let ws = match tokio_tungstenite::connect_async(&config.url).await {
            Ok((ws, _)) => ws,
            Err(e) => {
                tracing::warn!(target: "acp_hub::machine", error = %e,
        ...
        let ws = match handshake(ws, &config, &make_hello, &events, &handle, &mut cancel).await {
            HandshakeOutcome::Authenticated(ws) => {
                tracing::info!(target: "acp_hub::machine", "认证通过");
...
where
    F: Fn() -> (AuthSession, MachineHello) + Send + Sync,
{
    let (session, hello) = make_hello();
    let hello_frame = Frame::MachineHello(hello);
    let text = match serde_json::to_string(&hello_frame) {
        Ok(t) => t,
        Err(e) => {
            tracing::error!(target: "acp_hub::machine", "hello 序列化失败: {e}");
    ...
    if !sent {
        tracing::warn!(target: "acp_hub::machine", "hello 发送失败/超时，断开重连");
        return HandshakeOutcome::Retry;
    }
    ...
    match wait {
        Err(_elapsed) => {
            tracing::error!(target: "acp_hub::machine", "握手超时（{timeout:?}），断开且不自动重连");
        ...
        Ok(AuthWait::ProtocolViolation) => {
            tracing::warn!(target: "acp_hub::machine", "握手阶段收到非 auth_response 帧/关闭，断开重连");
            HandshakeOutcome::Retry
        }
        Ok(AuthWait::CloseConfigFatal) => {
            tracing::error!(target: "acp_hub::machine", "握手阶段 server 以 4502 关闭（配置性失败），停止重连");
...
fn audit_auth_failure(err: &AuthError) {
    tracing::error!(target: "acp_hub::machine", auth_failed_total = 1, reason = %err,
...
fn count_inbound_problem(err: &ProtoError) {
    match err {
        ProtoError::Unsupported(tag) => {
            tracing::warn!(target: "acp_hub::machine", tag, "入站未知/非 M1 帧（计数，丢弃）");
        }
        ProtoError::Malformed(e) => {
            tracing::warn!(target: "acp_hub::machine", error = %e, "入站畸形帧（计数，丢弃）");
        }
        ProtoError::DirectionRejected(tag) => {
            tracing::warn!(target: "acp_hub::machine", tag, "入站帧方向违反（计数，丢弃）");
        }
    }
}
            ...
            msg = read_half.next() => {
                match msg {
                    Some(Ok(Message::Text(t))) => {
                        match Frame::parse(&t) {
                            Ok(frame) => {
                                if m1_check(frame.tag(), Role::Machine, Direction::Outbound) == M1Check::Allowed {
