...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-09-11（compact 可见收益、预算恢复与持久化 provenance）
...
| 改 compact 失败后的历史恢复 | `src/session/exec/executor_helpers/v2_execute.rs` + `peri-acp/src/host/prompt.rs` | `CompactionCommitState`；Phase 8 flush；`intercept_immediate_command`；`finish_prompt_turn` | 取消/模型错误仍返回可信 canonical snapshot；writer 失败或 compact commit uncertain 停止使用热状态、保留已提交磁盘内容供冷恢复，禁止按新增 ID 回滚已提交摘要；ARC-COMPACT-001 |
...
| 改工具执行分发 | `src/agent/stages/act.rs` + `src/agent/stages/tool_dispatch.rs` | `run_act`；`dispatch_tools`（并发执行 + 写 transcript） | 有 tool_calls → 并发执行；canonical 结果提交后、after_tools_batch 前计入 tool-growth，含解析失败结果；PTC 内部调用不重复结算；无 → 最终回答 emit TextChunk + StateSnapshot |
