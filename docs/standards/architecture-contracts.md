...
- **Rule**：provider 的响应完成不等于 Agent 任务完成。完整协议响应以 `MaxTokens` 停止且无工具调用时，保留原始消息但不运行完成 hook，经现有队列有界续跑；恢复预算耗尽必须返回 `MaxTokens` 未完成终态，仍服从取消与总迭代预算。完整工具调用照常执行，续跑不得重放已执行工具；残缺参数仍由 provider 协议校验拒绝。可见增量后流中断是同类未完成分支：Model 以 `Interrupted { error, attempts, max_attempts }` 终态交付，不重试同一请求；Agent 保留部分正文与流式 message_id，未签名思考与半截工具不回灌，不重复渲染文本，不运行完成 hook，经可信 `model_runtime/stream_interrupted` Required Defer 继续。无正文时不得写入空 assistant 消息（空 text block 会被 provider 拒收），只保留续跑提醒；判空沿用 `MessageContent::is_empty()`，不得用 `trim()` 替代。恢复预算唯一来源为事件携带的 `RetryConfig::max_attempts`，包含首次中断响应，loop 内累计不重置；耗尽返回 `StreamRecoveryExhausted`，保留底层 ModelError 并经 ACP allowlist diagnostic 投影，仍服从取消与总迭代预算。Agent 终态经标准 ACP `PromptResponse.stopReason` 传递，print 不得将非 `EndTurn` 响应当作成功退出。JSON/stream-json 最终 result 保留停止原因、任务状态及错误标记，在部署清理和通知通道排空后发出；清理失败与任务终态分别表达，不能用队列瞬时为空证明尾事件已交付。
- **Verify**：`cargo test -p peri-model`、`cargo test -p peri-agent --lib stream_interruption`、`cargo test -p peri-acp-types stream_recovery_exhausted`、`cargo test -p peri-acp stream_recovery_exhausted`、`cargo test -p peri-agent --lib truncation_tests`、`cargo test -p peri-agent --lib loop_result_mapping`、`cargo test -p peri-acp --lib prompt_wire_response`、`cargo test -p peri-tui --lib test_prompt_with_response`、`cargo test -p peri-tui --bin peri -- cli_print`、`cargo test -p peri-tui --test print_exit`；真实 CLI 固定响应覆盖截断续跑成功、连续截断未完成及进程退出。
