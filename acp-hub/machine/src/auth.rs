//! machine 侧双向认证（§9.2）：hello 构造（nonce 每次连接新生成）+ auth_response
//! HMAC 校验 + 认证状态机。
//!
...
//! （[`AuthError`] 只携带失败类别）。
//!
//! 握手语义（§9.2 步骤 1–3）：machine 发起连接 → 发 `machine/hello`（含 token
...
use acp_hub_proto::hmac::{
    ...
    HmacError, SESSION_CONTEXT_LEN,
};
use acp_hub_proto::machine::MachineHello;
use acp_hub_proto::version::PROTOCOL_VERSION;

/// HMAC 派生 role 字符串（§9.2：仅 machine 连接走双向认证，取值恒为 `"machine"`）。
...
/// 认证失败原因（脱敏：不含 token/nonce/hmac/session_context 本体，§9.3）。
#[derive(Debug, Clone, PartialEq, Eq, Error)]
pub enum AuthError {
    /// token 非 44 字符 base64（或解码非 32B）。
    #[error("token 非法（需 {TOKEN_B64_LEN} 字符 base64）")]
    BadToken,
    /// auth_response 载荷结构非法（session_context 非 base64 32B 等）。
...
#[derive(Debug, Clone)]
pub struct HelloCtx {
    /// 机器 hostname（§4.5 `machine/hello.hostname`）。
    pub hostname: String,
...
impl AuthSession {
    ...
    /// 构造 `machine/hello`（§4.5）：token + 本次连接 nonce + hostname + caps +
    ...
    /// `key = derive_mac_key(token, "machine")`；
    /// `input = mac_input(nonce, session_context, PROTOCOL_VERSION, "machine")`。
    ///
    /// `session_context` 来自应答（server 生成，§9.2），base64 解码为 32B 后
    /// 作为 MAC 输入；`hmac` 的 base64 合法性/长度/匹配由 [`verify_mac`] 内建
    /// 常量时间比较处理。
    pub fn verify_auth_response(&self, resp: &AuthResponse) -> Result<(), AuthError> {
        let context: [u8; SESSION_CONTEXT_LEN] =
            base64::engine::general_purpose::STANDARD
                .decode(&resp.session_context)
                .map_err(|e| AuthError::Malformed(format!("session_context: {e}")))?
                .try_into()
                .map_err(|_| AuthError::Malformed("session_context 非 32B".to_string()))?;
        let input = mac_input(
            &self.nonce,
            &context,
