...
//!
//! 端到端：fake client（TUI）+ fake machine 均以真实 tokio-tungstenite ws
...
//! 断链语义（machine 断开 → turn interrupted + session gap）。
//!
//! epoch 约定：fake machine 上报 epoch=0（F4 聚合器 stream.epoch 以 0 起始；
//! 真实 machine 的 epoch=1 首事件问题为 F4 已知缺口，见输出遗留问题）。
...
/// 测试装配：临时目录 + 双 token（machine/full）+ Hub。
struct TestServer {
...
async fn start_server() -> TestServer {
    ...
    let machine_rec = token_store
        .generate(TokenRole::Machine, "local")
        .unwrap();
    ...
    TestServer {
        addr,
        machine_token: machine_rec.token,
...
/// fake machine：hello（nonce + 双向认证校验）→ 帧循环（spawn/JSON-RPC
/// 应答）。
async fn fake_machine_connect(server: &TestServer) -> (
    tokio_tungstenite::WebSocketStream<
        tokio_tungstenite::MaybeTlsStream<tokio::net::TcpStream>,
    >,
) {
    ...
    let hello = Frame::MachineHello(acp_hub_proto::machine::MachineHello {
        token: server.machine_token.clone(),
        hostname: "local".into(),
...
#[tokio::test]
async fn machine_handshake_hmac_verified() {
    let server = start_server().await;
    let _ = fake_machine_connect(&server).await;
}
...
async fn e2e_create_prompt_event_broadcast() {
    ...
    msink
        .send(Message::Text(
            serde_json::to_string(&Frame::MachineHello(
    ...
    // create 的 session_id 未知，订阅 registry 建立 ready；create committed
    // 后再订阅 chat:{sid}。
    csink
        .send(Message::Text(
            serde_json::to_string(&Frame::YsyncSubscribe(
    ...
    // machine 收 machine/spawn → 回 spawn_ack。
    let session_id = match next_frame(&mut mstream).await {
        Frame::MachineSpawn(s) => {
            assert_eq!(s.command_id, create_cid);
            msink
                .send(Message::Text(
                    serde_json::to_string(&Frame::MachineSpawnAck(
                        acp_hub_proto::machine::MachineSpawnAck {
    ...
    // machine 收 initialize（machine/forward 帧）→ 回 forward_ack（L1+L2，
    // §4.4 M1 合并）→ 经 machine/event 回 JSON-RPC response（§4.5：机器上行
    ...
    let init = match next_frame(&mut mstream).await {
        Frame::MachineForward(f) => f,
        other => panic!("expected forward(initialize), got {other:?}"),
    };
    assert_eq!(init.frame["method"], serde_json::json!("initialize"));
    msink
        .send(Message::Text(
            serde_json::to_string(&Frame::MachineForwardAck(
                acp_hub_proto::machine::MachineForwardAck {
    ...
    msink
        .send(Message::Text(
            serde_json::to_string(&Frame::MachineEvent(
                acp_hub_proto::machine::MachineEvent {
        ...
        .await
        .unwrap();
    // machine 收 session/new（machine/forward）→ forward_ack → response。
    ...
    match next_action_ack(&mut cstream).await {
        Frame::ActionAck(a) => {
            ...
            assert_eq!(a.session_id.as_deref(), Some(session_id.as_str()));
        }
    ...
    csink
        .send(Message::Text(
            serde_json::to_string(&Frame::YsyncSubscribe(
                acp_hub_proto::ysync::YsyncSubscribe {
                    docs: vec![acp_hub_proto::conn::DocId::chat(&session_id)],
                },
            ))
            .unwrap()
            .into(),
    ...
    // machine 收 prompt（machine/forward）→ forward_ack → 经 machine/event 回
    // response（L3，§4.4）。
    ...
    // server 注入的 turnId（§4.4 生成规则）：machine 侧 ACP 会话沿用同一 id，
    // 事件 turn_id 与聚合器幂等键对齐（§6.5）——后续事件帧必须携带它。
    let turn_id = prompt.frame["params"]["turnId"].as_str().expect("prompt carries turnId");
    msink
        .send(Message::Text(
            serde_json::to_string(&Frame::MachineForwardAck(
                acp_hub_proto::machine::MachineForwardAck {
    ...
    assert!(matches!(
        next_action_ack(&mut cstream).await,
        Frame::ActionAck(a) if a.status == acp_hub_proto::ack::AckStatus::Committed
    ));

    // ---- machine 上报事件（epoch=0）→ client 收 y-sync 广播 ----
    msink
        .send(Message::Text(
            serde_json::to_string(&Frame::MachineEvent(
                acp_hub_proto::machine::MachineEvent {
                    ...
                    frame: serde_json::json!({
                        ...
                        "payload": {"turnId": turn_id, "entryId": "t-e2e:assistant", "blockId": "b1", "text": "streamed reply"}
                    }),
                },
            ))
            .unwrap()
        ...
        .await
        .unwrap();
    // 客户端收到广播（ysync.update，chat doc）。
    let mut got_broadcast = false;
    for _ in 0..8 {
        match tokio::time::timeout(Duration::from_secs(5), cstream.next()).await {
            Ok(Some(Ok(Message::Text(t)))) => {
                match Frame::parse(&t) {
                    Ok(Frame::YsyncUpdate(u)) if u.doc == acp_hub_proto::conn::DocId::chat(&session_id) => {
                        got_broadcast = true;
                        break;
                    }
                    Ok(Frame::KeepAlive(_)) | Ok(Frame::Pong(_)) => continue,
                    Ok(other) => {
                        panic!("unexpected frame during broadcast wait: {other:?}");
                    }
                    Err(_) => continue,
                }
            }
...
/// 断链语义：machine 断开 → session 置 gap + 活动 turn interrupted（§8.2）。
#[tokio::test]
async fn machine_disconnect_gaps_session() {
    let server = start_server().await;
    // 建立 machine 连接并完成一个 create（复用 e2e 前半段）→ 简化：直接
    // 通过注册表登记 session，再断开 machine 验证 relay 清理。
    // 端到端断言经 Registry Doc 不可行（内部），这里验证 relay 层行为已在
    // relay_event_handler_test 覆盖；本测试验证 gateway 断开路径触发
    // on_machine_disconnect（session 状态 Gap）。
    let url = format!("ws://{}/", server.addr);
    let (mws, _) = connect_async(url).await.unwrap();
    let (mut msink, mut mstream) = mws.split();
    let nonce = generate_challenge_nonce();
    ...
    hub.sessions.register(&sid, "local", Some("t")).await.unwrap();
    hub.doc.open_session(&sid, "local", Some("t")).await.unwrap();
    hub.sessions.bind(&sid, "acp-disc").await.unwrap();
    hub.sessions.set_active_turn(&sid, "t1").await;
    let _ = hub;

    // 断开 machine（Close 帧）→ gateway 触发 on_machine_disconnect。
    msink
    ...
    for _ in 0..50 {
        ...
        if let Some(e) = server._hub_keep.sessions.entry(&sid).await {
            if e.state == crate::control::SessionState::Gap {
                gapped = true;
                break;
            }
        }
    }
    assert!(gapped, "session should be gapped after machine disconnect (§8.2)");
    let _ = mstream;
}
