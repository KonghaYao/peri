...
Cautious mode adds a plan review before coding. Dispatch a read-only reviewer (`plan` or `code-reviewer`) to critique `02-plan.md` and write `02-plan-review.md`. The plan is not approved until every critical issue is fixed or explicitly accepted by the main agent/user.
