...
| 打开全部资源（会话存储） | `src/context.rs` | `Resources::open`；`Resources::open_with`；`open_with_default`（注入默认路径的 seam）；`open_read_only` / `degradable_open_failure`；`Resources::thread_store` | 默认路径 `~/.peri/threads/threads.db`（`SqliteThreadStore::default_path` 与只读入口共用 `sessions::default_database_path`）；先写打开，写打开失败且属于可恢复占用（schema 锁被占、库文件/WAL 不可写）时降级为只读打开并记 `tracing::warn!`——「写打不开」不等于「历史读不了」，不再挡住进入；本构建不认识的 schema（`UnsupportedSchemaVersion`/`UnsupportedDatabaseSchema`）不降级（只读打开只探列形状，会绕过版本判定）；只读打开也失败时返回写打开的原错误；不使用共享临时数据库 fallback |
