...
正在运行。待恢复的精确代际只在协商了 `peri.sessionRecoveryV1` 的连接上停在只读
（由客户端确认后调用 `peri/session_reset_dirty`；声明了该能力却无法展示确认的客户端会永远
停在只读——它自己放弃了代理解除，见 issue 的已知后果）；没有确认交互的连接由宿主
直接解除该精确代际并取得所有权，同样记 warning。降级只是不带 owner：写入与执行仍要
owner，`session/fork` 与同一次准入内的二次取得不接受降级，独占语义不变。
...
`RecoveryRequired`；结清前代只有两条入口：协商了 `peri.sessionRecoveryV1` 的
客户端经用户显式确认后调用 `peri/session_reset_dirty`，或宿主对没有确认交互的连接
直接解除该精确代际后准入（见上节）。两条路径都只解除观测到的精确代际，也都不构成
“上一代已正常收尾”的证明：OS 锁释放不证明外部副作用结束，PID 不存在也不能单独
证明整个进程树已经终止。禁止 TTL 到期后直接抢占。
