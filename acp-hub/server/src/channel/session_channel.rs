//! SessionChannel：客户端连接归一化（架构 §4.6，设计稿 `f5-channel-control.md` §6）。
//!
//! 每个 client 连接一个 `SessionChannel`，承载连接级协议状态：
//!
...
pub struct ChannelDeps {
    ...
    /// machine 注册表。
    pub machine: std::sync::Arc<MachineRegistry>,
    /// session 注册表。
...
impl SessionChannel {
    ...
    async fn dispatch_action(
        ...
        // 与 session 解析——Load 的 session 可能不存在，语义仍是方法不支持
        // （防御路径原在 coordinator::exec_command，但 submit 的 session
        // 检查会提前返回 SessionNotFound，必须在此层拦截）。
...
#[cfg(test)]
#[path = "session_channel_test.rs"]
mod session_channel_test;
