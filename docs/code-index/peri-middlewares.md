...
| 改 Bash 会话所有权 | `src/middleware/terminal.rs` + `peri-agent/src/agent/async_tasks/shell.rs` | `BashTool::execute`、`ShellExecutionGuard` | 前台命令、超时转后台共享一个外部执行 owner；取消后确认进程清理才允许释放 session lease |
