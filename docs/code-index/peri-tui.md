...
| --- | --- | --- | --- |
| 改消息流渲染 | `src/kit/message_area/mod.rs` + `message_area/render/` + `grid.rs` | `MessageArea`；`render_tool_card_lines` → `project_tool_render_plan` → `resolve_tool_render_plan` → `render_tool_plan`；`GridSpec::grid_for` | 工具 presentation 只投影 label/summary/detail 候选，统一 resolve/render 路径消费 status/fold/prefix/duration；语义复制复用同一 plan；组件只读 VIEW_MODELS，截断/坐标按 Unicode 显示宽度 |
...
| 改事件消费（新增/变更事件） | `src/kit/acp_events/` + `acp_notifier.rs` + `acp_bridge.rs` | `dispatch_and_notify`（穷尽 match）；`AcpEventData`；`convert_agent_event`；渲染产出 `push_view_models` / `push_acp_state` | 终止事件必须离开 loading；root `usage_update` 每次解码为 session-enveloped `CacheUsageUpdated(Some/None)` 并替换最后观察值（missing/zero/inconsistent 显式 clear），TurnDone 前至多注入一次 cache coverage note；带 `sourceAgentId` 的 auxiliary usage 不得覆盖父 turn；TurnSuspended 保留 sample，prompt/reset/non-stale interrupt/failure 清理；契约 ARC-EVENT-001 |
