...
- 🔄 **Workflow D（P4b 类型隔离）**：TUI 当前 ~70 处 `peri_middlewares::` + ~29 处 `peri_agent::` 导入。已完成 dto_convert.rs 桥梁层（6 个 MCP 类型转换 + OAuth channel 桥接）、ServerInfoDto 同步、status_bar.rs McpInitStatus DTO 切换、events.rs OAuthCallbackResult DTO 重导出。v2 面板已在 P3 迁移时使用本地 DTO（无 peri_middlewares 依赖）。**剩余阻塞**：PermissionMode DTO 变体名不同（Plan/Yolo vs AutoMode/Bypass）、Plugin/HITL/AskUser/Cron DTO 需同步或新建、OAuthFlowEvent 含 oneshot channel 无法直接 DTO 化。**Legacy 面板（~40 处导入）待 P5 删除**。`scripts/check-tui-imports.sh` pre-commit 钩子未启用。
