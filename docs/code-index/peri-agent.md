...
| 改子 Agent 事件转发与关闭排空 | `src/agent/subagent_event_forwarder.rs` | `spawn_subagent_event_forwarder`；session 使用 `spawn_subagent_event_forwarder_for_completion` | session 的 Stop 交由 lifecycle owner 在 join 后提交；observe → render → state 的 biased 优先级保持；observe Closed 只停用自身分支，三通道全部关闭且排空才退出；保留 child source 注入与生命周期事件去重；`subagent_event_forwarder_test.rs` 覆盖 producer 先关闭后的缓冲交付（ARC-EVENT-001） |
