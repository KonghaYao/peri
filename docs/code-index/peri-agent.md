...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-09-09（compact 与循环退出语义校准）
...
| 改 compact 策略选择 | `src/agent/compact_v2/mod.rs` + `src/agent/stages/compact.rs` | `determine_compact_action(budget, config)`（mod.rs:102，Skip/Micro/Smart 选择）；`run_compact`（mod.rs:125 编排）；阶段入口 `stages/compact.rs::run_compact` | Micro 计划收益不足且 `budget_pct >= auto_compact_threshold`、`reclaim_target > 0` 时先提交 Micro 再尝试 Full；LLM 缺失由 Full 执行阶段报 `CompactNoLlm`，不是升级守卫；cache-aware 仅是 Micro 分支的提前跳过条件；`planner.rs::CompactPolicy::force_full_threshold` 无消费点（遗留） |
...
| 改 Micro/Full 执行细节 | `src/agent/compact_v2/{micro,projection,full}.rs` | `micro_compact`；`render_llm_view`；`re_inject_v2`、`extract_file_info`、`extract_skill_names` | Micro 不原地截断 transcript，而是持久化 message-level projection directive，后续模型视图由 `render_llm_view` 投影工具输入；精确载荷污染风险见 `spec/issues/2026-09-09-p0-micro-compact-edit-write-context-corruption.md`；Full 走 `peri_model::Model` 摘要 + re-inject |
