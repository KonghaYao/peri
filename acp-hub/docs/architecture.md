...
现有 `acp-hub` 是一个 stdio 桥接器：IDE stdin → JSON-RPC 解析 → 按 session 分流到独立 ACP 子进程 → 子进程 stdout 转发回 IDE。单机、单连接、进程随 IDE 生命周期。

### 1.2 演进目标

将 acp-hub 升级为**中心服务器**形态：

1. **server / machine 两级实体**：server 是中心控制面；machine 是实际运行 ACP 进程的机器，与 server 通过 WebSocket 联通，接收 server 下发指令完成 ACP 进程的启动/停止。
2. **ws 通信取代 stdio**：为未来 CS 远程模式（本机 TUI 连远程 server、局域网多机）打基础。
3. **yjs 统一数据对象**：ACP 事件在 server 侧经**规范化边界 + 聚合器（agg）**投影为**视图对象**，以 yjs 标准数据结构承载（每会话双 Doc + 全局 Registry Doc），多端（多 TUI、未来 Web 面板）经 yjs 同步一致。
4. **TUI 纯视图层**：server 是独立常驻后台进程；TUI 只是视图层，与 server 是 client–server 关系，经 ws + yjs 同步状态。

### 1.3 非目标（明确不做）

- session 跨 machine 迁移 / 自动恢复
- server 自动负载均衡调度
...
| P3 | server 崩溃/重启不中断 agent | 重启 server 后 machine 自动重连，agent 产出**在缓冲有界承诺内不丢**（缓冲上限内不丢；超限按 §8.5 丢弃策略丢弃并以 gap 呈现，不假装完整）【顾问：P0-3】 |
| P4 | machine 断线时活动 turn 明确中断，session 可恢复 | 断线瞬间活动 turn 呈现 `interrupted`；补推完成后 session 恢复可用、可开新 turn（见 §7.3 分区恢复裁决） |
| P5 | 新建 session 显式指定 machine（默认本机） | 路由可预测、可调试 |
...
| P9 | machine daemon 崩溃不产生无人知晓的孤儿执行 | 重连后 server 对「已中断但 machine 声称存活」的 session 默认下发 kill 清理，TUI 可见（见 §7.5） |
...
- **连接方向**：machine **主动 outbound** 连接 server（NAT 友好、server 零入站依赖）；TUI/Web 主动连 server。
- **单 ws 多路复用**：一条连接上按帧类型区分控制帧（Action/Ack）与状态帧（y-sync），见 §4。
- **machine 与 ACP 进程之间保持 stdio**：复用现有 `child.rs` 的 spawn/监控/转发能力（现有资产直接迁移），machine 对上层统一输出为**原始 ACP 帧流**。
- **规范化只发生在 server 侧**：`ACPChannel` 边界在 server（§6.1），machine 保持透明转发。
...
| 二进制 | 职责 | 备注 |
|--------|------|------|
| `acp-hub-server` | 常驻后台：认证、控制面、ACPChannel 规范化、聚合器、DocManager、machine 注册表 | 无 TUI 依赖，可 launchd/systemd 托管 |
| `acp-machine` | 每台机器一个：outbound 连 server、收 spawn/kill 指令、管理 ACP 进程树、透明转发 + 断线缓冲 | 由现有 acp-hub 单机版演化 |
| `acp-hub-tui` | 纯视图层：yjs 渲染 + Action/Ack 操作 | 与 peri-tui 视觉风格一致（ratatui），独立实现 |

共享 crate：`acp-hub-proto`（帧定义、Action/Ack 信封、machine 协议类型、Y.Doc schema 的 Rust 类型镜像）。

> 裁决依据：用户明确要求 server 与 machine 为两个独立二进制。共享代码收敛在 `acp-hub-proto`，避免两处重复实现协议解析。

### 3.3 machine 职责边界【审查：架构 P1-2 + 运维 P0-2】

machine 是 **dumb pipe**，但「不做协议理解」需精确化——缓冲分桶与转发要求最小协议面：

**允许的最小协议面（machine 侧唯一允许的 ACP 帧解析）**：
1. 双格式 sessionId 提取（原始 `{type,payload}` 与 JSON-RPC 包裹格式，与 §6.1 ACPChannel 双格式兼容规则一致）；
2. 按 session 分桶 + 分配单调 `seq`；
3. 进程管理（spawn/kill/退出监控）与本地缓冲。

**禁止项**：不解析事件语义（不区分 delta/终态/工具）、不聚合、不写任何状态、不生成业务事件。

**无法提取 sessionId 的帧**：丢弃并记本地缺口计数（随 `machine/hello` 上报）。

**machine 进程本身崩溃**【审查：运维 P0-2】：
- ACP 子进程随 daemon 死亡（沿用 `kill_on_drop` 语义，现 child.rs:45）——其上 session 全部中断（同断线语义，见 §7.5）；
- 内存缓冲与磁盘溢出缓冲**不跨重启保留**（重启后 `hello` 上报 `buffer_lost: true`）；
- 每 session `seq` 计数器与 `stream_epoch` 绑定（daemon 重启后 epoch +1、seq 可重置，§4.5.1【顾问：P0-2】）。
...
- 序列化：控制帧（action/ack/error/machine.*/event/keep_alive/ready/pong/auth）用 serde_json；`ysync.*` 帧体为 y-sync 协议消息（`Y.encodeStateAsUpdate` / update diff），**base64 嵌入文本帧**（与 chat `broadcaster.ts` 的 `Buffer.toString("base64")` 一致）；固定 update 编码版本 v1。【审查：开发 P2→写为协议事实】
...
| `ysync.subscribe` | C→S | `{ docs: ["chat:{sid}", ...] }` | 订阅指定 Doc 的更新（多 session 视图必需）【审查：开发 P1】 |
...
| `event` | S→C | `{ session_id, seq, frame }` | `events/subscribe` 推送（§4.3）【审查：开发 P1】 |
| `machine.*` | S↔M | machine 协议帧（§4.5） | server ↔ machine 专用 |
| `auth` | C→S | `{ token }` | 连接建立后的第一个帧；**角色由 token 解析，客户端不声明 role**【审查：开发 P2】 |

### 4.3 Action envelope 与方法面

参照 chat §7.1（Q9 修订）：客户端只发送 `commandId` 与 action 内容，信封其余字段由服务端按会话绑定补充与校验。

```jsonc
{
  "t": "action",
  "commandId": "uuid",          // 幂等键，同 session 唯一；重试复用同一 ID（绝不可换 ID 猜测结果）
  "type": "session/prompt",     // 见方法面
  "payload": { ... }            // sessionId 等由服务端按连接绑定补充，客户端字段不可覆盖 binding
}
```
...
| type | payload | 说明 |
|------|---------|------|
| `session/create` | `{ machine_id?, cwd?, title? }` | machine_id 缺省 = 本机（P5）；cwd 语义见下 |
| `session/load` | `{ session_id }` | 载入既有会话（转发 ACP `session/load`；转发前开启回放窗口，见 §8.6） |
| `session/close` | `{ session_id }` | 关闭并 kill 对应 ACP 进程（offline 时语义见 §7.6） |
| `session/prompt` | `{ session_id, message }` | 转发到目标 machine |
| `session/cancel` | `{ session_id }` | 转发 cancel（携带目标 sessionId，路由据此精确投递） |
| `permission/resolve` | `{ session_id, permission_id, decision }` | 权限应答（CAS 校验通过后才下发，见 §7.4） |
| `events/subscribe` | `{ session_id?, from_seq? }` | 原始 ACP 事件订阅（§4.3.1） |
...
- `events/subscribe { session_id?, from_seq? }`：`from_seq` 缺省 = 实时起（不重放历史）；带 `from_seq` 则从该序号起推。
- `events/unsubscribe { session_id? }` 退订。
- 推送帧：`{ "t": "event", session_id, seq, frame }`（frame 为规范化事件，sessionId 为 hub 侧 session_id，经 binding 翻译——不透传原始 acp_session_id）【审查：架构 P2-3】。
- 无权限 session 的订阅 → `FORBIDDEN`。
...
{ "t": "action_ack", "commandId": "uuid", "status": "accepted" | "committed" | "duplicate",
  "turnId?", "sessionId?", "committedProjectionVersion?" }
//  - sessionId：session/create 的 committed 必须携带（server 生成 id 的唯一告知路径）【审查：开发 P1】
//  - committedProjectionVersion：字段预留（对齐 chat types.ts，乐观并发校验二期启用）【审查：开发 P1】

// action_error
{ "t": "action_error", "commandId": "uuid",
  "code": "UNAUTHENTICATED" | "FORBIDDEN" | "SESSION_NOT_FOUND" | "MACHINE_OFFLINE"
...
- **去重记录必须移出 Y.Doc，持久化到 server 的 command outbox**【顾问：P0-1】。理由：去重要防的是 **ACP 进程的外部副作用**，而 Y.Doc 是可丢弃的实时镜像（§8.1 原则 5）——视图重建 ≠ 去重事实重建；且 update 日志会被 compact 裁剪、按磁盘预算归档，去重记录随 Doc 生命周期存亡会失效。outbox 是与 update 日志**并列的独立持久化文件**（按 session 分片，`commandId → {type, turnId, status, dispatched_at}`，与 §8.4 同一 fsync 纪律）。
- 去重表 = outbox 的内存索引：每 session 进程内 Map<commandId, 记录>，启动时从 outbox 重放重建；**committed 记录删除的唯一时机 = 显式清理策略**（如 session 关闭后保留 7 天、按磁盘预算淘汰，不随 Doc compact 消失）。P8 × P3 交集由 outbox 跨 server 重启成立。
...
| 层级 | 含义 | 达成条件 |
|------|------|---------|
| L1 ws 传输确认 | 指令帧已送达 machine | machine 收到下行指令并返回对应 `machine/*_ack`（spawn_ack/kill_ack）或对 prompt/cancel/resolve 的转发确认 |
| L2 stdin 写入确认 | machine 已将指令完整写入 ACP 进程 stdin | machine 侧写成功（字节级确认；ACP 子进程退出写失败 → 上报失败） |
| L3 ACP 接收确认 | ACP 进程已受理（JSON-RPC 请求已发出且未被连接错误拒绝） | machine 转发 ACP 的响应/错误帧（含 JSON-RPC error 也算受理，业务失败走 `action_error`） |

M1 实现 L1+L2 合并（machine 在转发确认中隐含写成功），L3 作为 `session/prompt` 的 committed 前置。
...
- **路径 B（不支持）**：L2 后未取得 L3 一律进入持久化 `delivery_unknown`——**非幂等命令（prompt/cancel/permission/resolve）禁止自动重试**，直至可观测状态对账（如 agent 状态查询）或人工裁决完成；仅幂等命令（create/close 按 session_id）可安全重发。
...
|--------|-------------|------------------|---------------|
| `session/create` | outbox 落盘后、spawn 前 | `dispatched=false` | 重发重新走 spawn（§6.2 时序；session_id 幂等，已存在返回现有句柄） |
| `session/create` | spawn 后、binding 前 | `dispatched=true` | 重发 → server 与 machine 对账 binding，缺失则重走；不二次起进程 |
| `session/prompt` | outbox 落盘后、投递前 | `dispatched=false` | 重发重新投递（客户端重试窗口内） |
| `session/prompt` | 投递后（L1+L2 达成）、投影前 | `dispatched=true` | 重发返回 `duplicate` + turnId，**不重复调用 Agent**（outbox 兜底，提交顺序不可倒置）；恢复时该记录按 delivery_unknown 裁决：路径 A 以关联 ID 查询，路径 B 展示「结果未知」并走对账/人工【顾问2】 |
| `session/cancel` | 同 prompt | 同 prompt | 同 prompt；`cancelling` 状态幂等迁移 |
| `permission/resolve` | 同 prompt | 同 prompt | 同 prompt；CAS 已迁移则返回 `duplicate`【审查：开发 P2】 |
| `session/close` | outbox 落盘后 | 同 prompt | 重发；machine offline 时走 §7.6 `pending_close` |

**重试分类**：`AGENT_UNAVAILABLE` / `MACHINE_OFFLINE` → `retryable=true`（可自动重试，须复用同一 commandId）；`INVALID_STATE` / `FORBIDDEN` / `SESSION_NOT_FOUND` → `retryable=false`（重试不会改变结果）。
...
### 4.5 Server ↔ machine 协议

**Server 下发**（下行指令均携带 `commandId`；以 `session_id` 为天然幂等键——server 可安全重发）【审查：架构 P1-4 + 开发 P0-3/P0-4】：

| 方法 | 参数 | 说明 |
|------|------|------|
| `machine/spawn` | `{ command_id, session_id, cmd, cwd, env? }` | 启动 ACP 进程；**按 session_id 幂等**（已存在返回现有句柄，不二次起进程）；`env` 受 server 白名单约束（§9.5）【顾问：P1-7】 |
| `machine/kill` | `{ command_id, session_id, grace? }` | 停止 ACP 进程；**幂等**（已死成功返回） |

**Machine 上报**：

| 方法 | 参数 | 说明 |
|------|------|------|
| `machine/hello` | `{ token, hostname, caps, buffered?, buffer_lost?, stream_epochs?, nonce }` | 注册 + 重连握手；**幂等替换语义**——新 hello 到达即 fencing 旧连接（旧连接事件丢弃、关闭）【审查：架构 P1-4】；`buffer_lost` 上报 daemon 崩溃缓冲丢失（§7.5）【审查：运维 P0-2】；`stream_epochs` 为 per-session 流纪元映射（§4.5.1）【顾问：P0-2】；`nonce` 用于 server 身份证明（§9.2） |
| `machine/heartbeat` | `{ load, alive_sessions }` | 周期心跳（默认 5s） |
| `machine/event` | `{ session_id, epoch, seq, frame }` | 原始 ACP 帧转发（**带 machine 侧单调 seq 与流纪元**）【审查：开发 P0-3】【顾问：P0-2】 |
| `machine/buffer_sync` | `{ session_id, epoch, from_seq, frames[] }` | 断线缓冲补推（frames 每帧带 seq；epoch 由 server 回传校验，见 §4.5.1） |
| `machine/spawn_ack` | `{ command_id, session_id, ok, error? }` | spawn 结果（成功/失败+脱敏原因）【审查：开发 P0-3】 |
| `machine/kill_ack` | `{ command_id, session_id, ok }` | kill 结果 |
| `machine/process_exit` | `{ session_id, code }` | ACP 进程退出事件（含退出码；`crashed`/`ended` 状态由此驱动）【审查：开发 P0-3】 |

**buffer_sync 起点（from_seq）**【审查：开发 P1】：server 持久化 per-session `last_seq`（随 update 日志，§8.4）；重连后 `from_seq = last_seq + 1`。**machine 保留环形滑窗**（最后 500 条，覆盖 server 崩溃前已收未落盘段）作为兜底：server 发现缺口时请求滑窗重发。

#### 4.5.1 stream_epoch（流纪元）【顾问：P0-2】

`stream_epoch` 是 machine 侧 per-session 的**流代际标识**：machine 为每个 session 的 ACP 输出流维护一个纪元号，**daemon 重启或 ACP 子进程重建时 +1**（session 新开为 1）。它解决「补推边界无法区分旧流残余与新流开始」的歧义：

- **epoch 相同**（同一代际）：server 已持久化 `(epoch, last_seq)`，补推按 `from_seq = last_seq + 1` 连续追平；
- **epoch 变化**（daemon 崩溃重启 / 进程重建）：旧流 seq 空间作废，**server 判定该 session 产生不可校准缺口**——session 保持 `interrupted` + `gap`（uncalibratable），不尝试按 seq 补推旧流；若 session 已终止（ended/closed）则无需处理；
- server 持久化 `(epoch, last_seq)` 对（随 outbox/update 日志同目录独立文件）；`machine/buffer_sync` 回传 epoch，与 server 记录不一致即拒绝该批（防旧纪元缓冲混入新纪元流）；
- `machine/event` 携带 epoch：epoch 与 server 记录不一致的帧直接丢弃并计数（防御性，正常路径下 hello 已对账）。

### 4.6 连接建立时序（快照先于操作）

参照 chat §4.1（Q13 实现差异：不采用 y-sync 增量握手，先推全量快照）：

1. 连接配额检查 → `auth`（token）→ 授权解析（角色/可访问 session 集合）。
2. 按订阅清单（`ysync.subscribe`）打开/恢复 Chat Doc、Session Doc 与 Registry Doc（首个客户端注册广播监听）。
...
| 关闭码 | 触发条件 | 客户端行为 |
|--------|---------|-----------|
| 4500 | 机器离线（`MACHINE_OFFLINE`） | 停止自动重连，展示手动重试 |
...
**判定性时间戳权威**【审查：架构 P2-4】：expiresAt（权限 5min）、心跳 30s 判定、取消 10s 超时等判定性时间戳**统一由 server 单一权威时钟生成与判定**；machine 只上报相对时序（seq），不参与判定。
...
|---------------------------|------|
| `action`（session/create、session/prompt、session/cancel、session/close、permission/resolve） | events/subscribe、session/load 帧**不进 M1**（session/load 由 M2 载入；events/subscribe 由 M3） |
...
| M1 帧集（server ↔ machine） | 说明 |
|---------------------------|------|
| `machine/hello` / `machine/heartbeat` / `machine/event` / `machine/buffer_sync` / `machine/spawn` / `machine/kill` / `machine/spawn_ack` / `machine/kill_ack` / `machine/process_exit` | 全量（M1 即完整 machine 面） |
...
3. 重放：同一 `machine/event` 流补推两次 → 视图无重复 entry/toolCall（§6.3 幂等键）；
4. 终态守卫：cancelled 后晚到 delta 丢弃；interrupted 后带序依据终态事件恰一次校准；
5. 崩溃恢复：kill -9 server → machine 缓冲 → 重启 → buffer_sync 追平（含 epoch 相同/变化两分支）；
6. 帧集白名单：未知 `t` → `UNSUPPORTED_FRAME`；客户端上行 `ysync.update` → 拒绝；
7. delivery_unknown【顾问2】：L2 后崩溃且无 L3 → 非幂等命令禁止盲重试（路径 B）；路径 A 的关联 ID 查询分支；
8. 双向认证【顾问2】：重放旧握手报文 / 错误角色 / 过期 challenge / 未知 machine 身份 → 拒绝并关闭连接 + 审计计数；
9. 错误脱敏【顾问2】：错误回显与日志在截断前剔除命令参数/env 值/认证材料；
10. outbox 归档保留【顾问2】：归档触发后，存在未裁决 outbox 记录的 session 不丢失去重与恢复数据；
11. delivery_unknown 重启保留【顾问3】：跨进程重启注入——L2 已确认、L3 未确认 → 重启后该记录保持 delivery_unknown、可查询、非幂等命令**不自动再次投递**；幂等命令仅按 session 标识安全重发；
...
2. 聚合视图 = 多端（TUI/Web）真正消费的形态；TUI 不关心原始事件序列，只关心「当前会话长什么样」。
...
| Doc | 名称 | 内容 | 更新频率 |
|-----|------|------|---------|
| Chat Doc | `chat:{session_id}` | 消息时间线、内容块、工具调用、turn 投影 | 高频（内容流） |
| Session Doc | `session:{session_id}` | 会话元信息、Agent 状态、能力、活动 turn、权限请求、agent 磁盘历史会话列表 | 低频（控制状态） |
| Registry Doc | `hub:registry` | machine 列表 + **活跃 session 摘要列表**（全局视图，acp-hub 特有） | 低频 |

拆分理由（chat §5.1 同源）：**隔离高频内容流与低频控制状态**，降低订阅与同步成本。三份 Doc 都是 ACP 进程运行态的实时镜像，不是持久化恢复源；跨文档更新按 ACP 会话内事件顺序应用，不依赖跨 Doc transaction。

**sessions 投影位职责裁决**【审查：架构 P1-5】：

- **Registry Doc `sessions`** = TUI 会话列表的**唯一权威源**：活跃 session 摘要（id/machine_id/title/status/gap/updated_at），由 **server 状态源单写**（session 生命周期事件驱动：create/binding/终态/close 时更新），不从 Session Doc 聚合。
- **Session Doc `sessions`** = 该 ACP 进程的**磁盘历史会话列表**（`session_list` 10s 轮询投影，chat §5.3 语义在本架构下的正确对应——每 session 一进程，返回的是 agent 侧历史），供 `session/load`/resume 历史浏览，与 Registry 的活跃会话**语义不同、互不替代**。
- §15 映射表该行标注差异（非「同构」）。
...
### 5.4 Session Doc schema（chat §5.3）

```rust
struct SessionDocRoot {
    schema_version: u32,            // 旧快照恢复时以版本判空幂等补结构
    projection_version: u32,
    session: SessionInfoProjection, // sessionId/title/status/active_turn_id/created_at/updated_at
    agent: AgentStatusProjection,   // instance_id/acp_session_id/status/capabilities/last_activity_at/public_error
    ...
    sessions: Map<String, SessionSummaryProjection>,  // agent 磁盘历史会话列表（10s 轮询全量同步，旧条目删除自愈）——与 Registry Doc sessions 语义不同（§5.2）
}
...
struct RegistryDocRoot {
    schema_version: u32,
    machines: Map<String, MachineView>,   // id/hostname/status(online|offline|unknown)/token_id
                                          // /registered_at/last_heartbeat/session_count
    sessions: Map<String, SessionSummary>,// 活跃会话摘要（id/machine_id/title/status/gap/updated_at）
...
- **唯一提交边界 = DocManager**【审查：架构 P1-3 + 开发 P0-2】：所有 Y.Doc 写入（聚合器投影、控制面状态迁移如 cancelling/interrupted/decision/标题、定时器 CAS）都必须经 DocManager 的进程内单写通道（§7.4 每 session 单写者）；任何路径不得绕过 DocManager 直写 yrs。去重记录**不进** Doc（§4.4 outbox）。
...
- **敏感信息不进 Y.Doc**（chat §5.3 同源）：密钥、内部错误、原始凭证、machine 连接信息、组织上下文不得进入文档；租户/角色上下文由服务端连接绑定提供，不由文档字段声明。
- **schemaVersion 与 projectionVersion 分离**（chat §5.4）：前者描述结构，后者描述镜像进度；服务端升级 schema 时对存活 session 以幂等结构初始化补齐（旧客户端忽略未知字段仍安全）。
...
**`ACPChannel` 是唯一协议边界，位于 server 侧。** machine 透明转发原始 ACP 帧；server 的 ACPChannel 将其规范化为统一事件（NormalizedEvent），聚合层只消费规范化事件，不接受私有帧类型。双格式兼容：原始 `{ type, payload }` 与 JSON-RPC `session/update`（含包裹格式）统一提取。
...
**可信 binding**：server 维护 `acp_session_id → session_id` 映射；ACP 帧携带的 sessionId 与 binding 不一致直接丢弃；`acp_session_id` 只用于协议投递，不能成为 Doc 名称/广播频道/缓存键（chat §6.2 规则 5）。

### 6.2 session 创建时序（spawn → binding）【审查：架构 P0-3】

M1 核心闭环，新增正式时序（现有 router.rs 的「spawn → initialize → session/new → 注册映射」三步流程的分布式映射）：

```
客户端 session/create
  → server 选 machine（显式/默认本机）
  → server 下发 machine/spawn { command_id, session_id, cmd, cwd }
  → machine 拉起 ACP 进程
  → machine 上报 machine/spawn_ack { ok | error }
  → （spawn 失败 → action_error AGENT_UNAVAILABLE(retryable)，去重记录清除）
  → server 经 machine 转发 initialize（透传 JSON-RPC，machine 保持 dumb）
  → server 经 machine 转发 session/new
  → machine 上报 session 创建结果（acp_session_id）
  → server 建立 binding（acp_session_id → session_id）
  → 此后该 session 的 ACP 帧才允许投影（binding 建立前到达的帧一律丢弃，§6.4 丢弃语义在此挂钩）
  → action_ack committed（携带 sessionId）
```
...
| 权限请求/解决/过期 | Session Doc `pending_permissions` | 按 `permissionId` upsert；决议写 `decision`（CAS，§7.4） |
| Agent status/capabilities/session info | Session Doc `agent`/`session` | 覆盖当前状态；能力未确认前保持不可用 |
| `session_list` 响应 | Session Doc `sessions` | agent 磁盘历史，全量同步（幂等，10s 轮询），响应中不存在的旧条目删除（自愈） |
| turn 终态（完成/失败/取消/中断） | Chat Doc entry + Session Doc active_turn | 终态立即写入；之后的同 turn 增量丢弃（**interrupted 例外见下**） |
...
- **`interrupted` 是可校准终态**：仅允许同 `turnId` 且**带补推序依据**（`(session_id, seq)` 单调，见 §8.5）的终态事件（`turn_completed`/`turn_failed`/`turn_cancelled`）将其**恰一次**迁移为实际终态；其余事件仍丢弃。守卫实现从「状态位判断」改为「状态位 + 重放序判断」。

### 6.4 顺序、微批次与事务边界（chat §6.4 + 审查修订）

- 同一 session 的 ACP 帧按收到顺序进入独立有界缓冲区，**绝不与其他 session 混批**；聚合器为**每 session 串行消费者**【审查：架构 P1-1】。
...
- **用户消息由服务端单写**：`session/prompt` 处理时 server 注册 `turnId` 并创建 user entry（幂等：同 `turnId` 重放跳过），ACP 的 `user_message_chunk` 增量以此映射。
...
- 心跳：machine 每 5s 发 `machine/heartbeat`；server 30s 未收到（可配置）→ 标记 `offline`。
- 重连：machine 侧自动指数退避重连，重连后 `machine/hello` 携带缓冲水位/`buffer_lost`/seq 状态，server 据此协调补推（§8.3）；**hello 是幂等替换**——新 hello 到达即 fencing 旧连接【审查：架构 P1-4】。
- **machine 离线即刻生效**【审查：开发 P1】：判定离线那一刻，该机**所有非终态 turn（accepting/running/awaiting_permission/cancelling）统一 → interrupted**，该 session 所有 pending 权限**批量 expired**（复用 chat `expireTurnPermissions`），聚合器更新 yjs，所有 TUI 同步可见。
...
- `session.active_turn`（turnId + turnStatus + updatedAt）是权威投影，前端由 `turnStatus` 派生展示状态；会话级扁平 status 枚举不承担展示语义。
- `cancelling` 非终态但输出已停止——用户已取消，晚到增量一律丢弃（§6.3 终态守卫）。

### 7.3 session 生命周期与分区恢复裁决【审查：架构 P0-2】

```
session/create 或 load ──► accepting ──► ... （turn 状态机驱动）
                              │
        ACP 进程退出 ──► ended（终态，视图保留供历史查看）
        用户关闭 ──► closed
        进程崩溃 ──► crashed
        machine 断线 ──► 活动 turn → interrupted（turn 级终态）
```

**分区恢复裁决（P4 的实现语义）**：

- **`interrupted` 是 turn 级终态，不是 session 级终态**。
- session 级状态独立演进：machine 分区期间 session 置 `gap`（补推缺口，§8.5）；**补推完成、seq 追平后清除 gap，session 恢复可用，可开新 turn**——用户可在原 session 继续对话。
- 补推完成后 `active_turn` 恢复规则：旧 turn 保持 `interrupted`（或按 §6.3 例外校准为实际终态），新 turn 正常投影。
- 若补推无法完成（缓冲丢失/缺口不可补）：session 保持 `gap`，TUI 提示「载入以校准」（`session/load` 显式重建），不假装完整。
- 每次用户输入 = 新 turn；`session/prompt` 创建 turn（`accepting`），ACP 确认后 `running`。

### 7.4 并发规则（chat §8.2 + 审查修订）

1. 同一 session 的命令按**有界队列严格串行**执行（上限默认 64，超出返回 `RATE_LIMITED`）；串行性由进程内队列保证。
2. 默认每会话**仅一个活动 turn**；若未来支持并行 turn，必须先引入独立 branch/thread 聚合，不能直接放宽约束。
...
**每 session 单写者（Y.Doc 写入串行化）**【审查：开发 P0-2】：

- 存在三个写入路径：machine 事件聚合器（§6.4 独立任务）、command-coordinator 执行路径（user entry/权限 CAS）、权限超时定时器（CAS 迁移）。**yrs 的 `transact_mut()` 对同一 doc 的并发事务会 panic**（tokio 多线程下无互斥即崩溃）。
- 强制约束：**每 session 一个 writer task**——聚合器、命令写入请求、定时器 CAS 请求全部经该 session 的 mpsc 通道串行执行（`&mut DocPair` 独占）；等价实现为 per-session `tokio::sync::Mutex<DocPair>`。
- 跨 Chat/Session 双事务顺序**固定 chat → session**；禁止跨 await 持有 Y.Doc 事务。
- 命令入队检查（outbox 去重索引 + 队列上限）与 `in_flight` 标记必须在**同一临界区**内完成（chat 靠 JS 单线程天然原子，Rust 无此保证），否则并发重发可绕过去重表。

### 7.5 machine daemon 崩溃故障卡片【审查：运维 P0-2】

| 步骤 | 行为 |
|------|------|
| daemon 崩溃 | ACP 子进程随 daemon 死亡（kill_on_drop）；缓冲（内存+磁盘）全部丢失；per-session `stream_epoch` 递增（§4.5.1）【顾问：P0-2】 |
| machine 重启重连 | `hello` 上报 `buffer_lost: true` + 存活 session 清单 + 新 `stream_epochs` |
| server 裁决 | 对「已标记 interrupted 但 machine 声称存活」的 session：**默认下发 `machine/kill` 清理孤儿进程**，Registry 标记「已清理」，TUI 可见；不得静默保留 |
| 验收 | M1 验收矩阵补「kill -9 machine daemon」演练（P9） |

### 7.6 machine offline 时 session/close（pending_close）【审查：开发 P1】

- machine offline 时 `session/close` 无法下发 `machine/kill` → 返回 `MACHINE_OFFLINE`（retryable），视图标记 **`pending_close`**（Registry session 状态）；
- machine 重连后 server 对 `pending_close` 集合自动补发 `machine/kill`，完成后清标记；
...
1. **server 无状态化**：server 只持有元状态（Y.Doc + machine 注册表），不持有任何 agent 运行态。server 崩溃 ⇏ agent 中断；TUI 崩溃 ⇏ 任何影响。
...
6. **判定性时间戳由 server 时钟**（§4.7），machine 只上报相对时序。
...
| **machine 网络分区**（agent 还活着） | 不删除 Doc；活动 turn → interrupted（可校准）；session 置 gap | machine 重连后缓冲补推（§8.3）；seq 追平后清除 gap、session 恢复可用（§7.3） |
| **machine daemon 崩溃** | 同上（agent 随 daemon 死）；hello 上报 buffer_lost | server 对「已中断但声称存活」的 session 默认 kill 清理（§7.5） |
| **ACP 进程退出/被杀**（ended/closed/crashed） | 终态写入视图，Doc 保留供历史查看（归档策略见 §8.4） | 不再接受该 session 的新事件；缓冲清理 |

### 8.3 server 崩溃 / 重启（P3）

1. server 崩溃瞬间：machine 检测到 ws 断开。
2. machine 上的 ACP 进程**继续运行**；daemon 将原始 ACP 帧写入本地缓冲（内存，超限溢出到磁盘；上限默认 10MB/万条，可配置）。**「产出不丢」是有界承诺**【顾问：P0-3】：缓冲上限内不丢；超限按 §8.5 丢弃策略丢弃（delta 优先、控制帧最后），并以 `gap` 结构化呈现缺口——**不承诺无限缓冲**，避免「10MB 与产出不丢」矛盾表述。
3. machine 以指数退避重连 server。
4. server 重启后：加载持久化 Y.Doc（§8.4）与 **command outbox（重建去重索引，§4.4）**【顾问：P0-1】→ 各 machine 重连 `machine/hello`（含存活 session 清单、缓冲水位、`buffer_lost`、`stream_epochs`）→ **epoch 相同**的 session 按 `machine/buffer_sync` 补推（`from_seq = last_seq + 1`，环形滑窗兜底）；**epoch 变化**的 session 判不可校准缺口（§4.5.1）【顾问：P0-2】→ ACPChannel 规范化 → 聚合器按 §6.3 幂等规则重放（重放安全由 turnId/entryId/toolCallId 幂等键 + interrupted 校准例外保证）→ 视图校准。
...
- **数据目录磁盘预算**：日志 + compact 快照 + 缓冲文件总量上限（默认 2GB），超出触发告警 + 最旧 session 归档。
- **归档临时默认值**【审查：运维 P1-4 + 顾问2：P1-1】：已结束/关闭 session 的 Doc 保留 90 天（或按磁盘预算压缩/导出提示）；归档策略的正式方案为开放问题 3。**归档与 outbox 保留解耦**【顾问2：P1-1】：视图历史（update 日志/Doc 快照）可裁剪，但**命令账本（outbox）不得在存在未裁决记录、可恢复 machine 关系或仍开放控制面时删除**——删除前置条件：session 关闭 + machine 注销/不再重连 + outbox 全终态（completed/failed/delivery_unknown 已裁决）+ 保留期届满，缺一不可；90 天归档不得独立触发 outbox 清理。
...
- 持久化单元是**单文件内的单条记录**：update 日志按追加顺序、outbox 按 commandId 索引、`(epoch, last_seq)` 独立小文件——三者之间**不提供跨文件原子性**（chat→session 双 Doc 投影同理）；
...
4. **machine 对账后开门**：Registry 恢复为 `unknown` 状态，machine 重连（hello）后才转为 online/offline 并触发 §8.3 对账；对账完成前 session 视图允许展示但禁止新 `session/prompt` 之外的控制操作（防基于未对账状态的错误指令）；
5. **任一不变量失败**：进入 `degraded`（§17.2），可继续服务只读视图，拒绝新 committed 承诺（§8.4 落盘失败语义同源）。

**降级行为**：chat 与 session 双 Doc 中仅一个成功落盘时，允许视图短暂不一致（chat 有内容、session 无 agent 状态），**下一个控制事件 flush 时收敛**（§6.4 控制类先 flush），不允许恢复逻辑把两个 Doc 当原子对处理。

### 8.5 缓冲与补推契约（审查修订）

- machine 缓冲按 session 分桶，帧带单调 `seq`（machine 侧分配）与 `stream_epoch`（daemon 重启/进程重建后 +1，§4.5.1）；**daemon 重启后 seq 可重置**——此时 epoch 变化，补推契约失效，session 判不可校准缺口，不再按 seq 补推旧流【顾问：P0-2】。
- 补推按 `(session_id, epoch, seq)` 按序发送；**from_seq 起点 = server 持久化的 per-session `last_seq + 1`（epoch 相同前提下）**；machine 环形滑窗（最后 500 条）兜底 server 崩溃前已收未落盘段。
- **补推纪律**【审查：架构 P1-1】：重连后 **machine 先排空 `buffer_sync` 再恢复实时转发**（machine 侧保证）；补推完成前聚合器对该 session 暂停实时应用。seq 只承担补推路径排序，**实时路径排序 = server 到达序**；两路径经聚合器每 session 串行消费者合并（§6.4）。
- **gap 升级为结构化标记**【审查：开发 P1】：`gap: { count: u32, last_seq: u64, uncalibratable?: bool }`（缺口帧数、断点、是否可校准）；补推完成、seq 追平后**清除**（由聚合器写回）；`uncalibratable` 由 epoch 变化触发（§4.5.1），只能经 `session/load` 显式重建消除【顾问：P0-2】。
- 缓冲超限丢弃：**delta 类帧优先丢弃、控制帧/终态帧最后丢弃**【审查：运维 P1-6】；单帧大小上限（默认 1MB，超限直接跳过并记 gap）；「内存 + 磁盘合计 10MB」为计数口径。
- 缓冲文件权限 `0600`（内容是会话正文）；session 结束/清理时同步删除缓冲文件。
- 分区期间的 HITL 类交互（如权限请求）悬挂在 machine 侧缓冲，恢复后补推；产品语义明示。
...
- Action payload 上限 1 MB；每 session 命令队列上限 64。
- **回放窗口**（10s）：`session/load`/`resume` 转发时开启，承接先于 JSON-RPC result 到达的历史回放流；窗口外或 Chat Doc 已有内容时保持聚合层拒绝语义。
...
- server 下发 spawn/kill 到 machine 等价于**远程代码执行**能力 → machine 侧必须认证 server，server 侧必须认证 machine。
- 会话内容（密钥/代码）会出现在 Y.Doc 与事件流中 → 客户端必须认证，陌生设备不可见。
...
| token | 用途 | 权限 |
|-------|------|------|
| **machine token**（每机器一个） | machine 注册与认证 | 收 spawn/kill 指令、上报事件/心跳 |
...
1. machine 发起连接 → 发送 `machine/hello`（含 token + **一次性 challenge_nonce**，32B CSPRNG，每次连接新生成）。
2. server 校验 token（含协议版本、角色匹配）→ 响应 `HMAC(machine_token 派生密钥, challenge_nonce ‖ session_context ‖ protocol_version ‖ role)` 作为 server 身份证明。
3. **machine 校验通过前不执行任何 spawn/kill**；校验失败即断开（关闭码 4502 + 审计计数）。
...
- **会话绑定**：HMAC 输入绑定该连接唯一的 `session_context`（如连接级随机 id），响应无法跨连接重放；旧连接被 hello 幂等替换 fencing（§4.5），重放旧握手报文无效；
- **角色与版本绑定**：HMAC 输入含 role 与 protocol_version，防止跨角色/跨版本重放；
- **身份隔离**：每 machine 独立 token/派生密钥（§9.2.1 按机器签发），machine 之间密钥不可互换；
...
- MAC 输入字段规范化：`challenge_nonce ‖ session_context ‖ protocol_version ‖ role` 按**固定字节序**拼接（各自长度前缀 + UTF-8 编码；challenge/session_context 为 32B 原始字节），字段顺序即文档顺序，不得重排；
- 比较：常量时间比较（`subtle` 或等价实现），杜绝时序侧信道；
- 密钥：`machine_token` 经 HKDF 派生单连接密钥（派生上下文含 role），token 本体不出现在 MAC 输入；
- 测试向量必须以**字节级**定义（给定 nonce/context/version/role → 期望 MAC 输出），跨实现可验证。

#### 9.2.1 token 运维流程【审查：运维 P1-5】

- 生成：32B CSPRNG；machine 侧存储同样 `0600`。
...
- **token 即身份**：M1 无用户概念；一个 token = 一个身份（machine 或 client），无账号、无会话登录、无用户级 ACL；
- **授权面 = token 角色**：`machine` 角色可收 spawn/kill 指令；`full` client 角色可读全部 Doc + 发 Action；`read-only` client 角色仅读（§9.2.2）。**没有 session 级细粒度授权**——持有 client token 即可访问 server 上全部 session（单用户本地/家庭局域网前提下的显式取舍）；
- **非 loopback 拒绝策略**：M1 默认监听 `127.0.0.1`；显式配置局域网监听（M2）时，配置文件中声明 `allow_non_loopback: true` 才接受非回环连接——默认拒绝，防误暴露（§16）；
- **边界声明**：多用户、配额、session 级共享、审计合规均不在 M1–M3 范围（§1.3），演进到 M4 公网前必须重新评估授权模型，**不得在现有模型上打补丁**。

### 9.6 spawn.env 白名单【顾问：P1-7】

`machine/spawn` 的 `env` 参数（客户端 `session/create` 可间接传递）**不开放任意键**：
...
- machine 侧对 spawn 指令携带的 env 再校验一次白名单（双端校验，防 server 配置漂移）。
...
- 纯 client：连接 server（单 ws 多路复用），本地维护 **Y.Doc 只读镜像**（server-authoritative，不上行 update，§5.6）【顾问：P0-4】，渲染源 = Chat Doc / Session Doc / Registry Doc；操作 = Action/Ack。
...
| 区域 | 数据源 | 说明 |
|------|--------|------|
| 机器列表 | Registry Doc `machines` | 在线/离线、session 数 |
| 会话列表 | **Registry Doc `sessions`（唯一权威源，§5.2 裁决）** + Session Doc `sessions`（agent 磁盘历史，resume 浏览） | 状态、machine、标题、`gap` 徽标（含缺口计数） |
| 会话详情 | Chat Doc `entries`/`tool_calls` + Session Doc `active_turn` | 消息视图 + 工具卡片 + 权限请求；订阅经 `ysync.subscribe`（§4.2） |
...
| machine 上的 ACP 进程 | 默认 `peri acp`，可配置为任意符合 ACP 的 server |
...
├── proto/                # acp-hub-proto：帧/Action 信封/machine 协议/Y.Doc schema 类型镜像
...
│   ├── src/channel/      # gateway（ws 生命周期）、session-channel（action 归一化）、
│   │                     #   command-coordinator（串行队列+commandId 去重持久化）、
│   │                     #   relay-event-handler（machine 入站）、broadcaster（fan-out+背压）、
│   │                     #   connection-registry（配额）
│   └── src/persist/      # update 日志（blob+CRC32+compact）+ command outbox（§4.4）+ (epoch, last_seq) 水位
├── machine/              # acp-machine：child.rs（现有资产迁入，补进程组 kill）、缓冲、重连
├── tui/                  # acp-hub-tui：视图层
...
| 里程碑 | 范围 | 验收 |
|--------|------|------|
| **M1 本机闭环** | server + machine 同机 + tui + 三 Doc + token（含双向认证）+ 断线韧性 + **部署包** | P1–P9 全绿；TUI 崩溃重启不影响 agent；双 TUI attach 一致；**kill -9 server / kill -9 machine daemon 演练**【审查：运维 P1-1】；**§4.8 测试向量 1–10 全绿**【顾问2】 |
| **M2 局域网** | machine 部署到第二台机器、心跳/离线/重连、机器列表 UI、显式调度、**可观测性指标落地** | 断网 → turn interrupted 呈现 → 重连缓冲补推校准 → session 恢复可用；gap 计数可见 |
...
- **升级流程**：先升 server、后升 machine；`machine/hello` 携带 `protocol_version`（proto crate 定义），版本不匹配拒绝连接并报错，替代人肉纪律。
...
| 事件序与聚合一致性 | 重连补推乱序/重复 | 补推纪律（排空后恢复实时）+ `(session_id, seq)` 按序 + turnId/entryId/toolCallId 幂等 + interrupted 校准例外 + gap 计数（P0 契约测试固化） |
| 缓冲无限增长 | machine 断线期间事件堆积 | 内存 + 磁盘溢出 + 上限丢弃（delta 优先）+ gap 呈现 |
...
| Y.Doc 膨胀 | 长会话消息堆积 | 有损聚合（截断/摘要）+ 资源引用 + 归档（90 天临时默认） |
...
| 多文件持久化不一致 | chat/session 双 Doc 与 outbox 无跨文件事务 | 恢复不变量顺序修复 + degraded 降级（§8.4.1）【顾问：P0-5】 |
...
3. 归档策略正式方案（临时默认：结束 session 保留 90 天 / 磁盘预算压缩，§8.4）。**裁决方向**【顾问2：P1-1】：正式方案必须与 outbox 保留解耦——视图历史可裁剪，命令账本按未裁决状态保留（§8.4）。
...
|------------------|-------------|---------|
| `protocol/acp-channel.ts`（入站规范化） | `server/src/protocol/acp-channel` | 同构；输入来自 machine 转发的原始 ACP 帧而非直接 relay |
...
| `state/doc-manager.ts`（doc 生命周期+16ms 微批次） | `server/src/state/doc-manager` | 同构；acp-hub 增加 Registry Doc 与每 session 单写者约束（§7.4） |
| `state/permission.ts`（权限 CAS） | `server/src/state/permission` | 同构 |
| `state/session-list.ts`（10s 轮询全量同步） | `server/src/state/session-list` | **差异**：投影目标是 agent 磁盘历史（§5.2 裁决），与 chat「实例级会话列表」语义不同；活跃会话列表由 Registry 单写 |
...
| `channel/relay-event-handler.ts`（入站消费+断链清理） | `server/src/channel/relay-event-handler` | 入站源从 relay 变为 machine ws；断链语义差异见 §8.2 |
...
| Chat Doc / Session Doc schema | 同 schema（§5.3/§5.4） | Registry Doc 为 acp-hub 新增；**Chat Doc 不含去重记录**（acp-hub 去重记录在 outbox，§4.4）【顾问：P0-1】 |
...
| machine | 在线/离线数、缓冲水位（每 machine）、**缓冲溢出字节数与丢弃帧数（delta/控制帧分类）**、buffer_lost 次数 |
| 同步 | 初始同步耗时、gap 计数与缺口帧数、补推重放耗时与 backlog、实时/补推乱序丢弃数 |
| 聚合 | 聚合队列深度、微批次延迟、晚到丢弃数、去重命中数 |
| 持久化 | 落盘失败次数、fsync 耗时、compact 次数与耗时、**数据目录磁盘占用** |
| 会话 | session 创建/binding 成功率与耗时、turn 终态分布、权限请求/过期/决议计数 |

### 17.2 Degraded 判定规则（Registry Doc `global.status`）

以下任一触发 `Degraded`：落盘失败 / 缓冲溢出丢弃 / 任一存活 session 存在 gap / 镜像失败（聚合器异常）/ **启动恢复不变量失败（§8.4.1）**【顾问：P0-5】。`Restarting` 仅在 server 启动回放期间。判定规则集中实现于 server 状态源，TUI 状态栏呈现。
...
| 5 | 二进制形态 | server 与 machine 两个独立二进制（共享 proto crate） |
| 6 | machine 接入 | machine 主动 outbound 连接 + token 注册 + 心跳 |
| 7 | 断线语义 | machine 断线 → 其上 session 标记 interrupted，绑定不迁移 |
| 8 | session 调度 | 显式指定 machine + 默认本机 |
| 9 | yjs 边界 | ACP 事件聚合为视图对象进 yjs；原始事件走独立订阅 |
| 10 | 通道组织 | 单 ws 连接多路复用（Action/Ack + y-sync） |
| 11 | 认证模型 | server 签发 token，machine/client 角色分权 |
| 12 | server 韧性 | server 崩溃 agent 继续跑；machine 缓冲补推 |
| 13 | 聚合参考实现 | 聚合逻辑参照 `@fenix/chat-channel`（用户指定） |
| 14 | 文档拆分 | 每 session 双 Doc（Chat/Session）+ 全局 Registry Doc（chat §5.1 同源） |
...
| 17 | interrupted 校准【审查】 | interrupted 为可校准 turn 级终态；session 级 gap 可恢复（§7.3） |
...
| 19 | 单写者与提交边界【审查】 | 每 session writer task 串行化；DocManager 为唯一提交边界（§5.6/§7.4） |
| 20 | 双向认证【审查】 | machine hello 携带 nonce，server 以 HMAC 应答；校验前不执行指令（§9.2） |
| 21 | machine 崩溃语义【审查】 | 缓冲不跨重启 + buffer_lost 上报 + 孤儿进程默认 kill 清理（§7.5） |
| 22 | command outbox【顾问】 | 去重记录移出 Y.Doc 独立持久化；delivery_confirmed 三级（L1 ws/L2 stdin/L3 ACP）；崩溃点×重试行为表（§4.4） |
| 23 | stream_epoch【顾问】 | per-session 流纪元，daemon 重启/进程重建 +1；epoch 相同补推 last_seq+1，变化 → 不可校准 gap（§4.5.1） |
...
| 26 | 恢复不变量【顾问】 | outbox 先行 → last_seq 对齐 → Doc 补齐 → machine 对账后开门；任一失败降级 degraded（§8.4.1） |
| 27 | minimal IDL 与授权收窄【顾问】 | MVP-M1 帧集白名单 + 测试向量（§4.8）；M1 token 即身份 + spawn env 白名单 + 非回环默认拒绝（§9.5/§9.6） |
| 28 | HMAC 双向认证保留【顾问，否决删减】 | advisor 建议删除（链路级信任替代），被否决——server→machine 验证在明文 ws 阶段防冒充，M4 wss 是补充非替代（§9.2） |
| 29 | delivery_unknown【顾问2】 | L3 依赖 peri ACP 关联 ID 能力，M1 前二选一裁决：路径 A（支持）关联 ID 查询 / 路径 B（不支持）非幂等命令禁止盲重试 + 对账/人工；未裁决前按路径 B 实现（§4.4） |
| 30 | HMAC 协议级规范【顾问2】 | 双向随机 challenge + 会话绑定 + 单次使用窗口 + 角色/版本绑定 + 身份隔离 + 轮换路径；HMAC 不提供机密性，M4 强制 TLS（§9.2） |
| 31 | 归档与 outbox 解耦【顾问2】 | 视图历史可裁剪；命令账本按未裁决状态保留，删除前置条件四合一（session 关闭 + machine 注销 + outbox 全终态 + 保留期届满，§8.4） |
