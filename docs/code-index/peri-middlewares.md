...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-08-17（mcp/client、skill_discovery 拆分子模块；middleware/assembly/skills 行号刷新）
...
| 改 MCP 连接 / 初始化 | `src/mcp/client.rs` + `src/mcp/client/transport.rs` + `src/mcp/initialize.rs` + `src/mcp/reconnect.rs` + `src/mcp/client/subscription.rs` + `src/mcp/client_oauth.rs` | `McpClientPool`（client.rs:171）；`run_initialize`（initialize.rs:17，合并配置 → 逐台 spawn）；`serve_client_auto`（client/transport.rs:20）/`spawn_stdio_transport`（transport.rs:61）/`build_http_transport`（transport.rs:100）；`reconnect`（reconnect.rs:16） | 超时 STDIO 10s / HTTP 30s / shutdown 5s（client.rs:229-231）；初始化完成前状态写入不产生上下线通知（首 turn 概览已覆盖，避免重复事件）；`status_change_text`（client.rs:209）生成逐台通知文本；`tools/list` 全量 schema 磁盘缓存：`list_all_tools_cached`（client.rs:540，仅当 `io.mcpp/server-cache-version` 声明且 `persistent_cache_allowed` 才跨进程复用、同版本命中跳过网络、版本缺失/变化必回源）、`tools_cache_eligible`（client.rs:573，版本+策略准入）、`invalidate_tools_cache`（client.rs:580，`notifications/tools/list_changed` 经 `spawn_subscription_loop` 订阅分支失效磁盘）：网络 RPC 不持 cache lock、安全策略拒绝时保持原始 `peer.list_all_tools()` 行为、`tools/list_changed` 未订阅时由版本比对兜底 |
...
| 功能 | 入口/关键点 |
| --- | --- |
| 连接 / pool | client.rs（McpClientPool :171、pending_changes :187、超时常量 :229-231）；client/transport.rs（serve_client_auto :20、spawn_stdio_transport :61、build_http_transport :100）；client/subscription.rs（资源订阅循环 setup_subscription :217 / build_subscription_filter :197）；initialize.rs（run_initialize :17）；reconnect.rs（reconnect :16）；transport.rs（TransportConfig :9） |
