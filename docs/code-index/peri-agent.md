...
| 改后台 Bash 输出与完成唤醒 | `src/agent/async_tasks/{shell_output,shell,manager,registry}.rs` + `src/session/async_router.rs` + `src/agent/stages/mod.rs` | `ShellOutputCapture`、`finalize_bg_shell`、`subscribe_activity`、`run_react_loop` | 两路输出从采集开始落盘；短 reminder 只引用文件并提示 Read；回调 panic 不跳过终态；registry 派生 watch 唤醒 idle 重查状态，通知先入队再提交终态 |
