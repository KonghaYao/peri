...
| --- | --- | --- | --- |
| 改 worktree 绑定、恢复与会话环境 | `src/host/workspace.rs` + `src/host/requests/{session_lifecycle,legacy_session}.rs` + `src/host/assemble.rs` | `SessionExecutionLease`；workspace lifecycle helpers | new/load/resume/fork 统一验证绑定，环境按会话 cwd 装配，Incomplete 保留资源与 lease；身份契约见 `session-workspace-identity.md` |
