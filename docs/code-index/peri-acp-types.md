...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-08-29（MessageContent 判空共享 policy）
...
- portable seam：`MessageContent` 类型与投影仍属于本 crate；`is_empty` 的纯判定委托 dependency-free `peri-turn-policy`，供 native 与 `peri-wasi` 共用，不把 WIT 类型带入契约层
