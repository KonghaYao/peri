...
import { chatHead, chatStatusSignal, closeChat, closingChat, connState, isTerminal, openProjectSession, openingSessionId, permissions, projectSessions, readOnly, selectedCid, selectedSessionId, turnActive } from '../store';
import { Button, Dialog, IconButton, Menu, MenuItem, Status } from '../../ui';
      ...
      <div class="chat-actions">
        ...
        <Menu open={menuOpen()} id={menuId} label="会话操作" trigger={() => menuTrigger} onClose={() => setMenuOpen(false)}>
          <Show when={terminal()} fallback={
            <MenuItem tone="danger" disabled={readOnly() || closingChat()} onClick={() => { setMenuOpen(false); setConfirmClose(true); }}>关闭运行实例</MenuItem>
          }>
            <MenuItem disabled={readOnly() || !!openingSessionId()} onClick={() => { const id = logical()?.id; setMenuOpen(false); if (id) openProjectSession(id); }}>重新打开会话</MenuItem>
          </Show>
        </Menu>
      </div>
    </Show>
    <Dialog open={confirmClose()} title="关闭当前运行实例" dismissible={!closingChat()} onClose={() => setConfirmClose(false)}>
      <div class="runtime-dialog">
        ...
        <div class="form-actions"><Button disabled={closingChat()} onClick={() => setConfirmClose(false)}>取消</Button><Button variant="danger" busy={closingChat()} onClick={() => closeChat(() => setConfirmClose(false))}>关闭实例</Button></div>
      </div>
    </Dialog>
  </header>;
}
