...
export class GraphService {
  ...
  async getGraph(maxCommits: number = 200) {
    try {
      ...
      const { stdout, exitCode: logExitCode } = await execGit(this.docsDir, [
        "log", "--all",
        "--format=%H|%h|%P|%an|%ad|%s|%D",
        "--date=short", "--topo-order", "-n", String(maxCommits + 1),
      ]);
      if (logExitCode !== 0) return { error: stdout };
      ...
      if (commits.length > 0) {
        try {
          const statusRes = await execGit(this.docsDir, ["status", "--porcelain"]);
          if (statusRes.stdout) {
            const statusLines = statusRes.stdout.split("\n").filter(Boolean);
            const files = statusLines.map(l => l.slice(3).trim());
            const statRes = await execGit(this.docsDir, ["diff", "--stat", "HEAD"]);
      ...
      // 合成 Stash 条目
      try {
        const stashListRes = await execGit(this.docsDir, ["stash", "list"]);
        if (stashListRes.stdout) {
          ...
          for (let i = stashLines.length - 1; i >= 0; i--) {
            ...
            try {
              ...
              const showRes = await execGit(this.docsDir, ["show", "-s", "--format=%H|%h|%P|%an|%ad|%s", "--date=short", stashRef]);
              if (showRes.stdout) {
      ...
      return { hasRepo: true, commits, remotes, remoteBranches, moreAvailable };
    } catch (err: any) {
  ...
  async getCommitDiff(hash: string) {
    try {
      if (!hash) return { error: "缺少 hash 参数" };
      if (!/^[a-f0-9]{7,40}$/.test(hash)) return { error: "无效的 hash" };
      const metaRes = await execGit(this.docsDir, ["show", "--format=%an|%ad|%s", "--date=short", "--no-patch", hash]);
      const metaParts = metaRes.stdout.split("|");
      const diffRes = await execGit(this.docsDir, ["show", "--format=", hash]);
      return { author: metaParts[0] || "", date: metaParts[1] || "", subject: metaParts[2] || "", diff: diffRes.stdout };
    } catch (err: any) {
      return { error: err.message };
    }
  }
