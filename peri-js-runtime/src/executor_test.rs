...
use super::{
    normalize_execute_response_error, JsExecutionLimits, JsExecutionRequest, JsExecutor,
    JsRpcRouter,
};
...
async fn test_execute_supports_concurrent_tool_promises_and_completion() {
    let result = JsExecutor::new("node")
        .execute(
            JsExecutionRequest {
...
async fn test_execute_rejects_source_over_limit_before_spawn() {
    ...
    let error = JsExecutor::with_limits("missing-node-program", limits)
        .unwrap()
        .execute(
            JsExecutionRequest {
...
async fn test_execute_hard_timeout_kills_busy_loop() {
    ...
    let error = JsExecutor::with_limits("node", limits)
        .unwrap()
        .execute(
            JsExecutionRequest {
