...
> 速查表：通用 JavaScript process/RPC host。细节以代码为准。更新：2026-09-09（macOS process-group cleanup race）。
...
| --- | --- | --- | --- |
| 改 Node 生命周期或 stderr | `peri-js-runtime/src/host.rs` | `JsExecutionHost::spawn`、`terminate_and_wait`、`recover_termination_race`、`kill`、`wait` | host 持有 child/channel/incoming，并并行消费 stderr；macOS group signal 的 zombie `EPERM` 竞态通过限时 reap + process-tree retry 收敛，仍保留真实 cleanup 错误 |
