...
| --- | --- | --- | --- |
| 改工作流 RPC 传输/挂起清理 | `src/rpc.rs` | `send_request`、`write_line`、`drain_pending`、agent register/deregister | pending 必须先于写入登记；每条 frame 写换行并 flush；stdout EOF/child 结束必须 drain pending；agent 句柄以所有权 token 防旧 task 删除新实例，契约 ARC-WORKFLOW-RPC-001 |
