...
impl FrozenSessionData {
    ...
    /// 同时保留 v1 兼容字段。
    #[allow(clippy::too_many_arguments)] // skills 端口为波 2 注入面新增，L5 迁出后可分组
    pub fn build(
        cwd: &str,
        language: Option<&str>,
        ...
        frozen_date: &str,
        permission_mode: peri_acp_types::permission::PermissionMode,
        workflow_enabled: bool,
        skills: &dyn peri_acp_types::ports::SkillsPort,
    ) -> Self {
        ...
        let system_prompt = template.render(&env, &features, skills, plugin_agent_dirs, language);
...
pub struct SessionContext {
    ...
    // ── controller: 控制面宿主（3.0 批 2）──────────────────────────────────
    // 执行发起（Controller::run_session）与事件发射（Controller::publish_event，
    // 事件三层化统一出口）经此宿主；事件泵从 Controller::subscribe 订阅。
...
pub async fn run_session_loop(ctx: SessionContext, turn: TurnInput) -> PromptResult {
    ...
    // 事件三层化：发射端把 /bg 子 agent 事件经 `Controller::publish_event` 发射
    // （Controller 补打 session_id/session_seq），消费端从 `Controller::subscribe`
                    ...
                    ev = bg_event_rx_for_cmd.recv() => {
                        match ev {
                            Some(bg_event) => {
                                ...
                                controller.publish_event(&bg_cmd_sid, &source, bg_event);
                            }
                            None => {
    ...
    // Registry → 事件链泵（事件三层化收尾）：发射经 Controller::publish_event
    // （BgRegistryEvent 包装为 ExecutorEvent::BgRegistryEvent 载体；身份降级为
    // 空串——registry 事件无 turn 归属），消费端从 Controller::subscribe 订阅
                ...
                tokio::select! {
                    ...
                    ev = registry_event_rx.recv() => {
                        match ev {
                            ...
                            None => {
                                // 发射点集合结束（sender 槽位被替换/关闭）：
                                // drain 广播在途事件后退出（与主 pump 同语义）。
                                loop {
                                    match subscription.try_recv() {
    ...
    // Main event pump（事件三层化：发射点 → Controller.publish_event →
    // 本泵订阅 Controller 消费；event_rx 仅作发射点集合的关闭信号）
    let (stop_reason_tx, stop_reason_rx) = exec_oneshot::channel::<PromptStopReason>();
    let pump_handle = spawn_event_pump(SpawnPumpRequest {
        subscription: ctx.controller.subscribe(),
        event_rx,
        stop_reason_rx,
    ...
    // 直发）经 `Controller::publish_event` 发射，事件泵随本轮 `event_tx` 关闭
...
struct ExecOutcome {
    ...
    agent_state: peri_agent::agent::state::AgentState,
}
...
async fn build_and_execute_agent(
    ...
    v2_message_queue: &peri_acp_types::session::MessageQueue,
    ...
    let (
        ...
        let frozen_data = FrozenSessionData::build(
            ...
            peri_acp_types::permission::PermissionMode::AutoMode,
    ...
    let register_runtime = ctx.session_manager.clone().map(|sm| {
        ...
        ) as peri_acp_types::frozen::RegisterRuntimeFn
    });
    let deregister_runtime = ctx.session_manager.clone().map(|sm| {
        ...
        }) as peri_acp_types::frozen::DeregisterRuntimeFn
    });
