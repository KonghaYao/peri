//! DocManager：唯一提交边界（§5.6）+ 每 session 单写者（§7.4）+ 16ms 微批次
//! （§6.4）+ 广播 + 唯一提交边界。
//!
//! 所有 Y.Doc 写入（聚合投影、控制面状态迁移、权限 CAS、定时器、Registry
//! 更新）都必须经 DocManager 的进程内单写通道；任何路径不得绕过 DocManager
//! 直写 yrs（§6.5）。yrs `transact_mut()` 并发 panic 由每 session 单写者排除。
...
use acp_hub_proto::schema::{
    ActiveTurnProjection, EntryStatus, MachineStatus, SessionStatus, SessionSummary, TurnStatus,
};
...
pub enum SubmitError {
    ...
    QueueFull,
    /// session 不存在 / 已关闭（SESSION_NOT_FOUND 语义）。
    SessionNotFound,
    /// 写者通道已关闭。
    ChannelClosed,
}
...
pub enum DocCommand {
    ...
    RegistryUpsertSession(SessionSummary),
    RegistryRemoveSession { session_id: String },
    /// Registry：machine 视图与全局状态（§12.4/§12.5）。
    RegistryUpsertMachine(acp_hub_proto::schema::MachineView),
    RegistrySetMachineStatus {
        machine_id: String,
...
impl DocCommand {
    ...
    fn is_registry(&self) -> bool {
        matches!(
            self,
            DocCommand::RegistryUpsertSession(_)
                | DocCommand::RegistryRemoveSession { .. }
                | DocCommand::RegistryUpsertMachine(_)
...
impl DocManager {
    ...
    pub fn with_recovered_registry(
        ...
        let (registry_tx, registry_rx) = mpsc::channel::<RegistryMsg>(cfg.session_queue);
    ...
    /// Registry 状态源单写句柄（channel 层 machine 生命周期 / 恢复流程使用）。
    pub fn registry(&self) -> RegistryState {
        RegistryState::new(self.registry_tx.clone())
    }

    /// 打开 session：Factory 创建双 Doc + ensure_schema（补结构）→ spawn writer
    ...
    pub async fn open_session(
        ...
        let (tx, rx) = mpsc::channel::<SessionMsg>(self.cfg.session_queue);
        ...
        let writer = tokio::spawn(session_writer_loop(
            session_id.clone(),
            rx,
            pair,
            cfg,
        ...
        if let Err(e) = registry.upsert_session(summary).await {
            tracing::warn!(session_id, error = ?e, "registry upsert session failed");
        }
    ...
    pub async fn release_reserve(&self, session_id: &str) {
        let sessions = self.sessions.read().await;
    ...
    pub async fn submit_event(&self, ev: NormalizedEvent) -> SubmitResult {
        ...
        let sessions = self.sessions.read().await;
        let Some(handle) = sessions.get(&session_id) else {
            return SubmitResult::Rejected(SubmitError::SessionNotFound);
        };
        ...
        if handle
            .tx
            .send(SessionMsg::Event(ev, Some(reply)))
            .await
            .is_err()
        {
            handle.inflight.fetch_sub(1, Ordering::SeqCst);
    ...
    /// `session_id` 路由到对应 session 写者。
    pub async fn submit_command(&self, session_id: &str, cmd: DocCommand) -> SubmitResult {
        if cmd.is_registry() {
        ...
        if handle
            .tx
            .send(SessionMsg::Command(cmd, Some(reply)))
            .await
            .is_err()
        {
            handle.inflight.fetch_sub(1, Ordering::SeqCst);
    ...
    pub async fn close_session(&self, session_id: &str) -> Result<(), DocManagerError> {
        ...
        let Some(handle) = handle else {
            return Err(DocManagerError::SessionNotFound(session_id.to_string()));
        };
        let (reply, rx) = oneshot::channel();
        let _ = handle.tx.send(SessionMsg::Shutdown(reply)).await;
        let _ = rx.await;
        let _ = handle.writer.await;
        // Registry 活跃摘要移除（§12.4 close 清理）。
        let registry = self.registry();
        if let Err(e) = registry.remove_session(session_id).await {
            tracing::warn!(session_id, error = ?e, "registry remove session failed");
        }
...
/// 每 session 写者循环（§7.4 单写者：`&mut DocPair` 独占；§8.3 微批次）。
async fn session_writer_loop(
    session_id: String,
    ...
    let (session_update_tx, mut session_updates) = mpsc::unbounded_channel::<Vec<u8>>();
    ...
    let _sub_session = SendSubscription(Some(
        pair.session
            .observe_update_v1(move |_, e| {
                let _ = session_update_tx.send(e.update.clone());
            })
            .unwrap_or_else(|e| panic!("session observe_update failed: {e}")),
    ));

    // 初始化全量基线落盘（chat → session 顺序，§6.4）：Factory 结构初始化
    // 发生在 observe 订阅之前（open_session），其 update 不会经回调产生——
    ...
    // 无害）。
    {
        ...
        if let Err(e) = sink.persist_update(DocId::chat(&session_id), init_chat).await {
            warn!(session_id, error = ?e, "chat init baseline persist failed");
        }
        let init_session = pair
            .session
            .transact()
            .encode_state_as_update_v1(&StateVector::default());
        if let Err(e) = sink.persist_update(DocId::session(&session_id), init_session).await {
            warn!(session_id, error = ?e, "session init baseline persist failed");
        }
    }
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                flush_batch(&session_id, &mut pair, &mut agg, &mut batch, &mut batch_bytes,
                    &mut chat_updates, &mut session_updates, &sink, &broadcast, &registry).await;
            }
            msg = rx.recv() => match msg {
                Some(SessionMsg::Event(ev, reply)) => {
                    if is_batchable(&ev.body) {
                        ...
                        flush_batch(&session_id, &mut pair, &mut agg, &mut batch, &mut batch_bytes,
                            &mut chat_updates, &mut session_updates, &sink, &broadcast, &registry).await;
                        let result = apply_event(&session_id, &mut pair, &mut agg, &ev,
                            &mut chat_updates, &mut session_updates, &sink, &broadcast, &registry).await;
                        if let Some(r) = reply {
                            let _ = r.send(result);
                        }
                ...
                Some(SessionMsg::Command(cmd, reply)) => {
                    // 控制类先 flush（§6.4）。
                    flush_batch(&session_id, &mut pair, &mut agg, &mut batch, &mut batch_bytes,
                        &mut chat_updates, &mut session_updates, &sink, &broadcast, &registry).await;
                    let result = apply_command(&session_id, &mut pair, cmd,
                        &mut chat_updates, &mut session_updates, &sink, &broadcast, &registry).await;
                    if let Some(r) = reply {
                        let _ = r.send(result);
                    }
    ...
    debug!(session_id, "session writer exited");
}
...
async fn flush_batch(
    ...
    session_updates: &mut mpsc::UnboundedReceiver<Vec<u8>>,
    sink: &Arc<dyn UpdateSink>,
    broadcast: &Arc<RwLock<Vec<mpsc::UnboundedSender<DocUpdate>>>>,
    registry: &RegistryState,
) {
    ...
    .await;
    // gap 上报（§9.4/§12.4）。
    report_gap(session_id, pair, registry).await;
...
/// 持久化 + 广播当前事务产生的增量 update（顺序 chat → session，§6.4）。
/// 返回是否全部落盘成功。
#[allow(clippy::too_many_arguments)]
async fn persist_and_broadcast(
    ...
    while let Ok(update) = chat_updates.try_recv() {
        ...
        if sink.persist_update(doc.clone(), update.clone()).await.is_err() {
            ...
            warn!(session_id, "chat update persist failed");
        }
        broadcast_send(broadcast, DocUpdate { doc, update }).await;
    }
    while let Ok(update) = session_updates.try_recv() {
        let doc = DocId::session(session_id);
        if sink.persist_update(doc.clone(), update.clone()).await.is_err() {
            all_ok = false;
            let _ = registry.report_condition(DegradeCause::PersistFailure).await;
            warn!(session_id, "session update persist failed");
        }
        broadcast_send(broadcast, DocUpdate { doc, update }).await;
    }
    all_ok
...
/// gap 上报：stream.gap_dirty → Registry sessions[].gap 写回（§9.4/§12.4）。
async fn report_gap(session_id: &str, pair: &mut DocPair, registry: &RegistryState) {
    ...
    trace!(session_id, gap_count = pair.stream.gap_count, uncalibratable = pair.stream.uncalibratable, "gap report");
    if let Err(e) = registry.set_session_gap(session_id, gap).await {
        // 上报失败不阻塞主流程；Registry 状态源会在下次机会补报。
        warn!(session_id, error = ?e, "gap report to registry failed");
    }
    // gap 存在 → SessionGap degraded 条件（§17.2）；追平 → 清除。
    if gap.is_some() {
        let _ = registry.report_condition(DegradeCause::SessionGap).await;
    } else {
        let _ = registry.clear_condition(DegradeCause::SessionGap).await;
    }
}
...
async fn apply_event(
    ...
    .await;
    report_gap(session_id, pair, registry).await;
    trace!(session_id, seq = ev.seq, kind = ev.kind(), applied = result.applied, reason = ?result.reason, "event applied");
    if !ok {
        SubmitResult::PersistFailed
    } else {
        SubmitResult::Applied(result)
...
async fn apply_command(
    ...
    let result = match &cmd {
        DocCommand::RegisterUserEntry {
            ...
            drop(txn);
            // session 侧：active_turn 注册（§7.2 accepting）。
        ...
        DocCommand::UpdateTitle { title } => {
            let mut txn = pair.session_txn();
            let root = txn.get_or_insert_map(crate::state::factory::ROOT);
            let session = root.get_or_init::<_, yrs::MapRef>(&mut txn, "session");
            session.insert(&mut txn, "title", title.clone());
            chat_writer::bump_projection_version(&mut txn, &root);
            ApplyResult {
                applied: true,
                reason: None,
        ...
        DocCommand::SetSessionTerminal { status } => {
            ...
            session.insert(
                &mut txn,
                "status",
                crate::state::aggregator::session_status_str(*status),
            );
            chat_writer::bump_projection_version(&mut txn, &root);
            ApplyResult {
                applied: true,
    ...
    .await;
    trace!(session_id, cmd = command_kind(&cmd), applied = result.applied, "command applied");
    if !ok {
        SubmitResult::PersistFailed
    } else {
        SubmitResult::Applied(result)
...
fn command_kind(cmd: &DocCommand) -> &'static str {
    match cmd {
        ...
        DocCommand::SetSessionTerminal { .. } => "set_session_terminal",
        _ => "registry",
    }
}
...
fn bump_session_projection(pair: &mut DocPair) {
    let mut txn = pair.session_txn();
    let root = txn.get_or_insert_map(crate::state::factory::ROOT);
    chat_writer::bump_projection_version(&mut txn, &root);
}
...
async fn registry_writer_loop(
    ...
    while let Some(msg) = rx.recv().await {
        match msg {
            ...
            RegistryMsg::SetSessionGap {
                session_id,
                gap,
                reply,
            } => {
                let r = applier.set_session_gap(&session_id, gap);
                persist_registry_updates(&mut update_rx, &sink, &broadcast).await;
                let _ = reply.send(r);
            }
            RegistryMsg::SetSessionStatus {
                session_id,
                status,
                reply,
            } => {
                let r = applier.set_session_status(&session_id, &status);
                persist_registry_updates(&mut update_rx, &sink, &broadcast).await;
                let _ = reply.send(r);
            }
        }
