...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-08-22（ToolSearch session-local 能力声明；Permission/HITL 拆分）
...
| 改 PTC / `run_code` | `src/ptc/mod.rs` + `peri-js-runtime/src/executor.rs`；canonical seam 在 `peri-agent/src/agent/stages/tool_dispatch.rs` | `RunCodeTool::invoke`；`PtcRouter::route`；`EffectiveToolDispatcher::dispatch` | `run_code` 为 ESM-only，Node module 使用 `await import('node:...')`；工具错误仅投影 stable code + fixed safe message，原始 source/input/exception/stack 不进入文本；默认可见且按 Bash 同级审批；普通 Node API 不经 `tools.*` Permission/HITL；内部 RPC 调用进入 canonical single-invocation seam并复用 effective-name permission/event/timeout/cancel |
