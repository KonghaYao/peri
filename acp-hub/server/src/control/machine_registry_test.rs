//! MachineRegistry 单测（设计稿 §16 测试 24–27）。
...
use acp_hub_proto::machine::{MachineForwardAck, MachineHello, MachineSpawn, MachineSpawnAck};

use super::*;
use crate::channel::OutboundMsg;
use crate::control::session_registry::SessionRegistry;
use crate::state::registry::RegistryState;

/// 无 Registry 的 SessionRegistry 替身（测试不写 Registry Doc）。
...
fn hello(machine_id: &str) -> MachineHello {
    MachineHello {
        token: "tok".into(),
...
async fn lifecycle_hello_heartbeat_offline() {
    ...
    let outcome = reg
        .on_hello("m1", "tok-1", MachineConn { tx }, &hello("m1"))
        .await;
    assert!(!outcome.fenced_previous);
    assert_eq!(reg.state("m1").await, Some(MachineState::Online));

    // 心跳续期。
    reg.on_heartbeat("m1", &MachineHeartbeat { load: 10, alive_sessions: vec![] })
        .await
        .unwrap();
    ...
    assert_eq!(reg.state("m1").await, Some(MachineState::Offline));

    // 重连心跳 → ONLINE。
    reg.on_heartbeat("m1", &MachineHeartbeat { load: 0, alive_sessions: vec![] })
        .await
        .unwrap();
    assert_eq!(reg.state("m1").await, Some(MachineState::Online));
    let _ = _drop;
...
async fn hello_fencing_replaces_connection() {
    ...
    reg.on_hello("m1", "tok-1", MachineConn { tx: tx1 }, &hello("m1"))
        .await;
    // 新 hello（同 machine_id）→ 旧连接 fencing（关闭信号）。
    let (tx2, _rx2) = mpsc::channel(8);
    let outcome = reg
        .on_hello("m1", "tok-1", MachineConn { tx: tx2 }, &hello("m1"))
        .await;
    assert!(outcome.fenced_previous);
    // 旧连接收到 Close(1011)。
...
async fn spawn_ack_tracking_and_timeout() {
    ...
    reg.on_hello("m1", "tok-1", MachineConn { tx }, &hello("m1"))
        .await;
    ...
    // machine 侧先收到 spawn 帧。
    match rx.recv().await {
        Some(OutboundMsg::Frame(Frame::MachineSpawn(_))) => {}
    ...
    let r = reg
        .on_ack(
            "m1",
            "c1",
            MachineAck::Spawn(MachineSpawnAck {
                command_id: "c1".into(),
                session_id: "s1".into(),
                ok: true,
                error: None,
    ...
    assert!(matches!(r2, Err(MachineError::Timeout)));
    let _ = _drop;
}

#[tokio::test]
async fn offline_machine_rejects_commands() {
    let (registry, _drop) = test_registry();
    let sessions = SessionRegistry::new(registry);
    let reg = MachineRegistry::new(
        Duration::from_secs(30),
    ...
    let (tx, _rx) = mpsc::channel(8);
    reg.on_hello("m1", "tok-1", MachineConn { tx: tx.clone() }, &hello("m1"))
        .await;
    // 断开（当前连接句柄）→ OFFLINE。
    assert!(reg.on_disconnect("m1", &MachineConn { tx }).await);
    let cmd = MachineSpawn {
        command_id: "c1".into(),
        session_id: "s1".into(),
        cmd: vec!["peri".into()],
        cwd: "/".into(),
...
async fn forward_rpc_and_offline() {
    ...
    let ack_task = tokio::spawn(async move {
        match rx.recv().await {
            Some(OutboundMsg::Frame(Frame::MachineForward(f))) => {
    ...
    assert!(matches!(
        ...
        Err(MachineError::UnknownMachine(_))
    ));
    // 断开后 OFFLINE。
    reg.on_disconnect("m1", &MachineConn { tx: tx.clone() }).await;
    assert!(matches!(
...
#[tokio::test]
async fn session_epoch_from_hello() {
    let (registry, _drop) = test_registry();
    let sessions = SessionRegistry::new(registry);
    let reg = MachineRegistry::new(
        Duration::from_secs(30),
    ...
    let (tx, _rx) = mpsc::channel(8);
    reg.on_hello("m1", "tok-1", MachineConn { tx }, &h).await;
    assert_eq!(reg.session_epoch("m1", "acp-1").await, Some(1));
    assert_eq!(reg.session_epoch("m1", "acp-2").await, None);
    let _ = _drop;
}

/// 孤儿清理钩子（§16 测试 27 + §7.5/§7.6）：意外存活（server 已标记终态但
/// machine 声称存活）+ pending_close 补发 → 下发 kill 断言（fake conn 收
/// kill 并回 ack）。
///
/// 注意：M1 `machine/hello` 无 alive_sessions 字段（§4.5 表），`on_hello`
...
async fn cleanup_orphans_kill_decision() {
    ...
    let sessions = SessionRegistry::new(doc.registry());
    // 下发超时 2s：ack 回填需在窗口内。
    let reg = MachineRegistry::new(
        Duration::from_secs(30),
    ...
    // §7.6）；s3：正常存活（不 kill）。
    sessions.register("s1", "m1", None).await.unwrap();
    sessions.transition("s1", SessionState::Closed).await.unwrap();
    sessions.register("s2", "m1", None).await.unwrap();
    sessions.request_close_offline("s2").await.unwrap();
    sessions.register("s3", "m1", None).await.unwrap();

    let outcome = HelloOutcome {
        fenced_previous: false,
        buffer_lost: false,
        alive_sessions: vec!["s1".to_string(), "s3".to_string()],
        session_epochs: Default::default(),
    };
    // cleanup 内 send_kill 等 ack：放独立 task，fake machine 侧回填。
    ...
    for _ in 0..2 {
        ...
        match msg {
            OutboundMsg::Frame(Frame::MachineKill(k)) => {
                targets.push(k.session_id.clone());
                let ack = MachineAck::Kill(MachineKillAck {
            ...
            other => panic!("expected MachineKill, got {other:?}"),
        }
    }
    ...
    assert!(sessions.pending_close_sessions().await.is_empty());
    let _ = (doc, tmp);
}
...
async fn fencing_stale_disconnect_keeps_new_connection_serving() {
    ...
    reg.on_hello("m1", "tok-1", MachineConn { tx: tx1.clone() }, &hello("m1"))
        .await;
    // 新 hello（fencing 旧连接，§4.5）。
    let (tx2, mut rx2) = mpsc::channel(8);
    let outcome = reg
        .on_hello("m1", "tok-1", MachineConn { tx: tx2.clone() }, &hello("m1"))
        .await;
    assert!(outcome.fenced_previous);
    // 旧连接退出（gateway 断链路径）——陈旧断开：不得置 Offline。
    assert!(!reg.on_disconnect("m1", &MachineConn { tx: tx1 }).await);
    assert_eq!(reg.state("m1").await, Some(MachineState::Online));
    // 新连接仍可服务：spawn 下发成功（fake machine 回 ack）。
    let spawned = tokio::spawn({
        let reg = reg.clone();
        async move {
            reg.send_spawn(
    ...
    match rx2.recv().await {
        Some(OutboundMsg::Frame(Frame::MachineSpawn(k))) => {
            assert!(reg
                .on_ack(
                    "m1",
                    &k.command_id,
...
/// （§4.5 表），对账输入唯一来源是 `machine/heartbeat`——alive 变化且非空
