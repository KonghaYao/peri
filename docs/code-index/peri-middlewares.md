...
| 改 ToolSearch 中间件注入 | `src/tool_search/middleware.rs` | `ToolSearchMiddleware`、`before_agent`、`prompt_contribution` | 优先读 v2 每 turn 本地工具视图（`state.local_tools()`），无则回退 shared_tools（v1/测试路径）；deferred 变化时重建；collect_tools 仅提供 SearchExtraTools + ExecuteExtraTool |
