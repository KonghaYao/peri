...
- 🔄 **Workflow D（P4b 类型隔离）**：~~TUI 当前 ~70 处 `peri_middlewares::`~~ → **活跃代码 28 处**（排除 legacy 面板 40 处 + test 8 处）。已完成 dto_convert.rs 桥梁层（MCP 7 类型 + InstallScope + SkillMetadata/SkillSource 转换 + OAuth channel 桥接）、SkillMetadataDto 同步 + 6 消费者迁移。**剩余活跃导入分类**：HITL/AskUser 运行时通道（8 处，含 oneshot channel 桥接需求）、PermissionMode（2 处，变体名完全不同）、MCP 运行时对象（McpClientPool/McpInitStatus/OAuthFlowEvent，3 处）、CronScheduler/CronTask（2 处，需 CronTaskDto）、acp_stdio/acp_server 后端路径（7+3 处，合理保留）、oauth_prompt parse_code_from_url（1 处）。**Legacy 面板（~40 处导入）待 P5 删除**。`scripts/check-tui-imports.sh` pre-commit 钩子未启用。
