...
> 速查表：把「我想做什么」映射到文件。细节以代码为准。更新：2026-09-10（host/session 职责拆分）
...
| 功能 | 文件 | 入口/关键点 |
| --- | --- | --- |
| 会话注册表/API | session/mod.rs | `AcpSession` / `SessionManager` 持有唯一 session 注册表；`new_session_with_id` / `ensure_session` / `close_session` / `cancel_session` 保留生命周期入口 |
