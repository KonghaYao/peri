...
| 改事件映射（ExecutorEvent → 协议） | `src/event/mapper.rs` + `src/event/mod.rs` + `src/dispatch/session_replay.rs` | `map_event`；`tool_result_content`；`AcpEvent` DTO | ToolEnd live/replay 使用标准 `failed`/`completed`，同时写标准 `ToolCallUpdate.content` 与兼容 `rawOutput`，失败空文本有安全 fallback；其他流式事件映射为标准 `SessionUpdate`，私有事件按 caps 门控；契约 ARC-EVENT-001 |
