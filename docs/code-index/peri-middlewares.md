...
| --- | --- | --- | --- |
| 改 hook 能力与上下文 | `peri-agent/src/middleware/{capabilities,trait}.rs` + 各 middleware `impl Middleware` | `BeforeAgentState` / `BeforeToolState` / `AfterToolState` / `AfterAgentState` / `BeforeModelState` / `CatalogState` / `StateView` | 输入替换只允许 before_agent；只读视图不泄漏 queue/catalog，MCP 通知/Goal/Stop/GitWatch 使用队列能力，ToolSearch 初始与 Reason 重绑使用 catalog/recall；能力适配不改变装配顺序或工具可见性 |
...
| 改 subagent 工具 / 继承 / 取消 | `src/subagent/tool/define.rs` + `configuration.rs` + `spawn_context.rs` + `src/subagent/fork.rs` | `SubAgentTool` / `BaseTool::invoke`；public builders、`host` 归 configuration，`spawn_config_base` / `resume_config_base` / lifecycle 归 spawn_context | 单一工具 owner；父 Session host 整体优先于 builder 回退；同步/后台取消与 frozen 经 Agent 层唯一 SessionFactory；父工具过滤归 fork.rs，事件按 source_agent_id 归属 |
...
| 改 SubAgent 定义来源 | `src/subagent/tool/definitions.rs` | `load_agent_def` / `load_agent_def_for_resume` / `read_definition` | 本地→内置→插件查找与 frozen built-in policy；文件读取/解析错误共享一入口；MCP 缓存读取不替代显式激活审批 |
| 改 HITL 提问通道（AskUser） | `src/hitl/mod.rs` + `src/ask_user/mod.rs` | `HumanInTheLoopMiddleware`（hitl/mod.rs:39，`tool_names` :45、`new` :74、`collect_tools` :90）；`parse_ask_user`（ask_user/mod.rs:31）；`ask_user_tool_definition`（:65） | 2026-08-15 职责拆分：本中间件只做提问通道（AskUserQuestion 工具 + 12_ask_user 段落），审批归 PermissionMiddleware；broker 恒 None 时不装配（workflow agent 无提问，`assembly/workflow.rs::build_middlewares`）；装配须用原始 broker（见多路复用行） |
...
| 改文件 / 终端 / Web / Todo / Image 工具 | `src/middleware/` | filesystem.rs（collect_tools :39）；terminal.rs（BashTool :21、TerminalMiddleware :480，collect_tools :534）；web.rs（WebMiddleware :7，WebFetchTool :38 / WebSearchTool :37）；todo.rs（TodoMiddleware :18，`new` 收 notify_tx :25）；image/（ImageMiddleware :26 + compressor.rs :30） | 纯工具提供器：collect_tools 注册 + 透传 is_direct；Todo 带通知通道；Image 按 before_agent 本批输入 ID 逐条处理 @image 附件转 ContentBlock::Image，保留既有附件块且不重读历史，以 `BeforeAgentState` 的 `replace_message` 能力保持原消息 ID，Agent before_agent runner 在链结束后回写 |
