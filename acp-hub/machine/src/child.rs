...
//!   （hub 组装 `machine/process_exit`）；
//! - **stdin 写**：`write_line` 写原样 JSON 行 + flush（§4.4 L2 的 machine 侧
//!   语义；进程已退出 → 写失败上报）。
//!
//! 进程组 kill 用 `kill(-pgid, sig)`（`libc` 符号）。`libc` crate 未预填
//! （见 f6-machine.md §12，由主管统一处理），本模块以自声明 FFI 落地同一
...
///
/// response 无 sessionId 字段，归属由 machine 已知（本进程唯一 hub session），
...
pub enum ChildOutput {
    ...
    Frame(ChildEvent),
    /// stdout EOF 后 wait 完成（hub 组装 `machine/process_exit`）。
...
pub async fn spawn(
    ...
    tracing::info!(target: "acp_hub::machine", session_id, pgid, "ACP 子进程已启动（进程组）");
...
async fn run_stdout_reader(inner: Arc<AcpInner>, tx: mpsc::UnboundedSender<ChildOutput>) {
        ...
        match stdout.read_line(&mut line).await {
            ...
            Ok(_) => {
                ...
                let parsed: serde_json::Value = match serde_json::from_str(trimmed) {
                    Ok(v) => v,
                    Err(e) => {
                        tracing::warn!(target: "acp_hub::machine", session_id = %inner.session_id,
                ...
                match extract_session_id(&parsed) {
                    ...
                    None => {
                        ...
                        if is_json_rpc_response(&parsed) {
                            ...
                            tracing::debug!(target: "acp_hub::machine", session_id = %inner.session_id,
            ...
            Err(e) => {
                tracing::error!(target: "acp_hub::machine", session_id = %inner.session_id,
                    "ACP stdout 读取错误: {e}");
                break;
            }
        }
    ...
    tracing::info!(target: "acp_hub::machine", session_id = %inner.session_id, code, "ACP 子进程退出");
...
async fn run_stderr_reader(inner: Arc<AcpInner>) {
    ...
    tracing::debug!(target: "acp_hub::machine", session_id = %inner.session_id, bytes, lines,
...
impl AcpProcess {
    ...
    pub async fn kill(&self, grace: Duration) -> anyhow::Result<()> {
        ...
        tracing::info!(target: "acp_hub::machine", session_id = %self.inner.session_id, pgid,
