...
| 批 4 | 测试迁移收尾（`create_test.rs` 17KB）+ 文档事实源（`docs/top-level.md` §7/§8、code-index）+ 依赖清理 | `cargo test --workspace` |
...
7. **id 保真结论（批 0 标注，未改行为）**：agent-client-protocol-schema 的 `RequestId` = `Null | Number(i64) | Str(String)`（`rpc.rs:42`，JSON-RPC 2.0 §5）；`transport::types::RequestId` = `String | Number(i64)`（无 Null）——内部域已覆盖可表示子集。`transport/stdio.rs:224-229` 对域外值（小数、u64 溢出 i64、null、bool 等）**静默压 0**（`as_i64().unwrap_or(0)`），非保真且与合法 id 0 存在碰撞风险；String id 与 i64 域内 id 保真。结论：**显式对齐约束**——域外 id 应按协议违规处理（拒绝或保留可区分标记），而不是压 0；批 0 以测试锁现有行为（`stdio_test.rs: test_request_id_conversion_*`），统一 stdio transport 时（批 1/2）再裁定收口。另记录：EOF 后 pump 退出仅是 `recv()` 侧感知关闭，已发出的 pending `send_request` 不会被自动失败/取消（无内置超时）——闭环时需要显式处理（§8.1，`stdio_test.rs: test_pending_request_waits_after_eof`）。