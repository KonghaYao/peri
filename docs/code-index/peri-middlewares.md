...
| 改 ToolSearch 中间件注入 | `src/tool_search/middleware.rs` + `peri-agent/src/agent/model_bridge.rs` | `ToolSearchMiddleware::before_agent` / `prompt_contribution`；`AgentModelBridge::build_request` | 优先读 v2 每 turn 本地工具视图（`state.local_tools()`），无则回退 shared_tools（v1/测试路径）；deferred 变化时重建；collect_tools 仅提供 SearchExtraTools + ExecuteExtraTool；主 bridge 与 StageContext 共享同一 chain，并在首个 Reason 的 ModelRequest 构造时读取 after-before_agent contribution，fresh stage 不复用旧 middleware cache |
