...
**与 Anthropic/Claude Code 博客的关系：** Peri 博客借鉴了 Claude Code 官方博客的教学式渐进结构（先讲概念定义，再讲为什么重要，最后讲怎么做）、关键术语 **bold** 强调、以及丰富的交叉引用习惯。但有三个刻意的差异——(1) Peri 的段落更短（2-4 句 vs 3-5 句），保持工程师的切分感；(2) Peri 的语气更直接，可以下判断、可以表现偏好，不完全追求 Claude Code 的临床式冷静距离感；(3) Peri 不回避第一人称（我们、我），用个人经历和具体场景驱动叙述。
...
- **引用外部项目（Claude Code、Aider 等）时，首次出现必须附 GitHub 或官网链接。** 例如「[Claude Code](https://github.com/anthropics/claude-code) 和 [Aider](https://github.com/Aider-AI/aider) 的早期版本」
...
9. **中文标点清零**——全文搜索中文冒号「：」和直角引号「」，逐一替换。项目地址行中的冒号是固定格式的例外不在此列。直角引号在任何情况下都不出现——要引用的短语直接说，不加任何引号。这是生成时最高频的机械性违规——写完第一稿后全局搜索这两个字符是性价比最高的自检。
